...
crystal_maiden_frostbite_custom.attack_range = 400
crystal_maiden_frostbite_custom.attack_duration = 1
crystal_maiden_frostbite_custom.attack_cd = 20

crystal_maiden_frostbite_custom.legendary_slow = -4
crystal_maiden_frostbite_custom.legendary_radius = 400
crystal_maiden_frostbite_custom.legendary_max = 10
crystal_maiden_frostbite_custom.legendary_interval = 1
crystal_maiden_frostbite_custom.legendary_damage = 3
crystal_maiden_frostbite_custom.legendary_damage_max = 10
...
crystal_maiden_frostbite_custom.lowhp_heal = 25
...
crystal_maiden_frostbite_custom.slow_attack = -80
crystal_maiden_frostbite_custom.slow_cast = -70
crystal_maiden_frostbite_custom.slow_duration = {1.5, 2, 2.5}
...
function crystal_maiden_frostbite_custom:OnSpellStart(new_target)
  ...
  if mod and mod:GetStackCount() >= self.legendary_max then 
    stun = 1
    target:EmitSound("Maiden.Frostbite_stun")
  end
  ...
  target:AddNewModifier( caster, self, "modifier_crystal_maiden_frostbite_custom", {dispell = dispell, stun = stun, duration = duration } )
...
function modifier_crystal_maiden_frostbite_custom:OnDestroy()
if not IsServer() then return end
if not self:GetCaster():HasModifier("modifier_maiden_frostbite_3") then return end

self:GetParent():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_crystal_maiden_frostbite_custom_slow", {duration = (self:GetAbility().slow_duration[self:GetCaster():GetUpgradeStack("modifier_maiden_frostbite_3")])})
end
...
function modifier_crystal_maiden_frostbite_custom_tracker:IsHidden() return true end
...
if (self:GetParent():GetAbsOrigin() - params.attacker:GetAbsOrigin()):Length2D() > self:GetAbility().attack_range then return end
...
params.attacker:AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_crystal_maiden_frostbite_custom_attack_cd", {duration = self:GetAbility().attack_cd})
params.attacker:AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_crystal_maiden_frostbite_custom", {duration = self:GetAbility().attack_duration*(1 - params.attacker:GetStatusResistance())})
...
local enemies = FindUnitsInRadius( self:GetCaster():GetTeamNumber(), self:GetParent():GetAbsOrigin(), nil, self:GetAbility().legendary_radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, 0, false )
...
if #enemies > 0 and self.ring == nil and self:GetParent():IsAlive() then 
  ...
  ParticleManager:SetParticleControl(self.ring, 2, Vector(self:GetAbility().legendary_radius, self:GetAbility().legendary_radius, self:GetAbility().legendary_radius ))
  ...
  ParticleManager:SetParticleControl( self.effect_cast, 1, Vector( self:GetAbility().legendary_radius, self:GetAbility().legendary_radius, 1 ) )
...
if (self:GetParent():GetAbsOrigin() - self:GetCaster():GetAbsOrigin()):Length2D() > self:GetAbility().legendary_radius then 
  ...
  if self:GetStackCount() < self:GetAbility().legendary_max then 
    ...
    if self:GetStackCount() >= self:GetAbility().legendary_max and self.mark == nil then 
...
if self:GetStackCount() >= self:GetAbility().legendary_max then 
  bonus = self:GetAbility().legendary_damage_max
end

return self:GetAbility().legendary_damage * self:GetStackCount() + bonus
end



function modifier_crystal_maiden_frostbite_custom_legendary_slow:GetModifierMoveSpeedBonus_Percentage()
return self:GetStackCount()*self:GetAbility().legendary_slow
end


function modifier_crystal_maiden_frostbite_custom_legendary_slow:OnTooltip()
return self:GetAbility().legendary_max
end
...
function modifier_crystal_maiden_frostbite_custom_slow:GetModifierPercentageCasttime()
  return self:GetAbility().slow_cast
end
