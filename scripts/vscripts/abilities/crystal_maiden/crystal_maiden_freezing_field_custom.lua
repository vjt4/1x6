...
LinkLuaModifier( "modifier_crystal_maiden_freezing_field_custom_bkb", "abilities/crystal_maiden/crystal_maiden_freezing_field_custom", LUA_MODIFIER_MOTION_HORIZONTAL )
...
if arcane and mod then 
	self:GetCaster():AddNewModifier(self:GetCaster(), arcane, "modifier_crystal_maiden_freezing_field_custom_bkb", {duration = arcane.spells_bkb})
	mod:Destroy()
end
...
function modifier_crystal_maiden_freezing_field_custom:OnCreated()	
	...
	self.shard_move = self:GetAbility():GetSpecialValueFor("shard_move")
...
if not IsServer() then return end
	...
	if self:GetCaster():HasShard() and (self:GetCaster():IsStunned() or self:GetCaster():IsSilenced() or self:GetCaster():IsHexed() or not self:GetCaster():IsAlive() or self:GetCaster():GetForceAttackTarget() ~= nil) then 
		self:Destroy()
	end
...
modifier_crystal_maiden_freezing_field_custom_bkb = class({})
function modifier_crystal_maiden_freezing_field_custom_bkb:IsHidden() return true end
function modifier_crystal_maiden_freezing_field_custom_bkb:IsPurgable() return false end

function modifier_crystal_maiden_freezing_field_custom_bkb:GetEffectName()
    return "particles/items_fx/black_king_bar_avatar.vpcf"
end

function modifier_crystal_maiden_freezing_field_custom_bkb:GetEffectAttachType()
    return PATTACH_ABSORIGIN_FOLLOW
end

function modifier_crystal_maiden_freezing_field_custom_bkb:CheckState()
    return {
        [MODIFIER_STATE_MAGIC_IMMUNE] = true
    }
end

function modifier_crystal_maiden_freezing_field_custom_bkb:DeclareFunctions()
    return {
        MODIFIER_PROPERTY_MODEL_SCALE
    }
end

function modifier_crystal_maiden_freezing_field_custom_bkb:GetModifierModelScale()
return 20
end

function modifier_crystal_maiden_freezing_field_custom_bkb:GetStatusEffectName()
    return "particles/status_fx/status_effect_avatar.vpcf"
end

function modifier_crystal_maiden_freezing_field_custom_bkb:StatusEffectPriority()
    return 99999
end
