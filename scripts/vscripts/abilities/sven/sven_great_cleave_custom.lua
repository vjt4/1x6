...
sven_great_cleave_custom.speed_bonus = {6, 9, 12}
sven_great_cleave_custom.speed_max = 5
sven_great_cleave_custom.speed_duration = 4


sven_great_cleave_custom.legendary_width = 150
sven_great_cleave_custom.legendary_distance = 1200
sven_great_cleave_custom.legendary_speed = 1600
sven_great_cleave_custom.legendary_damage_min = -20
sven_great_cleave_custom.legendary_damage_max = 60
sven_great_cleave_custom.legendary_cd = 8
sven_great_cleave_custom.legendary_cd_inc = 7
sven_great_cleave_custom.legendary_slow = -100
sven_great_cleave_custom.legendary_slow_duration = 1

sven_great_cleave_custom.stack_max = {15, 30}
sven_great_cleave_custom.stack_range = 700
sven_great_cleave_custom.stack_bva = {1.7, 1.6}
sven_great_cleave_custom.stack_damage = 2
sven_great_cleave_custom.stack_timer = 4

sven_great_cleave_custom.parry_reduce = -40
sven_great_cleave_custom.parry_cd = 12
sven_great_cleave_custom.parry_cd_inc = 7
sven_great_cleave_custom.parry_duration = 2
sven_great_cleave_custom.parry_heal = 6

sven_great_cleave_custom.lowhp_health = 50
sven_great_cleave_custom.lowhp_regen = -25
sven_great_cleave_custom.lowhp_duration = 2
sven_great_cleave_custom.lowhp_move = 15
...
if self:GetCaster():HasModifier("modifier_sven_cleave_7") then 
    if IsClient() then 
        return self.legendary_distance - self.legendary_width
    else 
        return 999999
    end
end
...
local k = self.legendary_cd_inc*(math.min(1, self:GetCaster():GetUpgradeStack("modifier_sven_great_cleave_custom")/626))
...
ProjectileManager:CreateLinearProjectile({
        ...
        fStartRadius = self.legendary_width,
        fEndRadius = self.legendary_width,
        vVelocity = (point - caster:GetAbsOrigin()):Normalized() * self.legendary_speed,
        fDistance = self.legendary_distance - self.legendary_width,
        Source = caster,
        ...
        iVisionRadius = self.legendary_width,
...
local k = math.max(0, math.min(1, distance/(self.legendary_distance*0.9) ))



target:AddNewModifier(self:GetCaster(), self, "modifier_sven_great_cleave_custom_legendary_slow", {duration = self.legendary_slow_duration*(1 - target:GetStatusResistance())})

--ApplyDamage(  { victim = target, attacker = self:GetCaster(), damage = damage, ability = self, damage_type = DAMAGE_TYPE_PHYSICAL, })
...
if not IsServer() then return end
if not self:GetParent():HasModifier("modifier_sven_cleave_7") then return end
...
if self:GetParent():HasModifier("modifier_sven_cleave_6") and
 ...
 and params.unit:GetHealthPercent() <= self:GetAbility().lowhp_health
 and not params.inflictor then 

    params.unit:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_sven_great_cleave_custom_healing", {duration = self:GetAbility().lowhp_duration})
    self:GetParent():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_sven_great_cleave_custom_healing_move", {duration = self:GetAbility().lowhp_duration})
...
self.damage = self:GetAbility().legendary_damage_min + table.k*self:GetAbility().legendary_damage_max
end
...
function modifier_sven_great_cleave_custom_combat_stacks:GetModifierBaseAttackTimeConstant()
if self:GetStackCount() >= self:GetAbility().stack_max[1] then 
    if self:GetStackCount() >= self:GetAbility().stack_max[2] then 
        return self:GetAbility().stack_bva[2]
    else 
        return self:GetAbility().stack_bva[1]
    end 
end
return
end
...
return self:GetAbility().lowhp_move
end
