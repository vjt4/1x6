...
sven_storm_bolt_custom.str_damage = {0.3, 0.45, 0.6}

sven_storm_bolt_custom.cd_range = {100, 150, 200}
sven_storm_bolt_custom.cd_stun = {0.2, 0.3, 0.4}

sven_storm_bolt_custom.legendary_aoe = 420
sven_storm_bolt_custom.legendary_duration = 10
sven_storm_bolt_custom.legendary_interval = 1.2
sven_storm_bolt_custom.legendary_stun = 0.15
sven_storm_bolt_custom.legendary_damage = 0.1
sven_storm_bolt_custom.legendary_stack = 2
sven_storm_bolt_custom.legendary_max = 8
sven_storm_bolt_custom.legendary_stack_damage = 0.05
...
sven_storm_bolt_custom.silence_duration = 1.2
sven_storm_bolt_custom.silence_slow = -100
sven_storm_bolt_custom.silence_mana = 40
...
function sven_storm_bolt_custom:GetBoltDamage()
...
if self:GetCaster():HasModifier("modifier_sven_hammer_7") then 
	...
	local line_position = hTarget:GetAbsOrigin() + hTarget:GetForwardVector() * self.legendary_aoe
	for i = 1, 6 do
...
local bolt_damage = self:GetBoltDamage()
...
for _,enemy in pairs(enemies) do
	if (not enemy:IsMagicImmune()) and not enemy:IsInvulnerable()  then
		ApplyDamage(  { victim = enemy, attacker = self:GetCaster(), damage = bolt_damage, ability = self, damage_type = damage_type, })
...
ParticleManager:SetParticleControl(self.zuus_nimbus_particle, 1, Vector(self:GetAbility().legendary_aoe, 0, 0))
...
local enemies = FindUnitsInRadius( self:GetParent():GetTeamNumber(), self:GetParent():GetOrigin(), nil, self:GetAbility().legendary_aoe, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + DOTA_UNIT_TARGET_FLAG_INVULNERABLE + DOTA_UNIT_TARGET_FLAG_OUT_OF_WORLD, 0, false )
...
for _,enemy in pairs(enemies) do 
	...
	if not enemy:IsInvulnerable() and (not enemy:IsMagicImmune() ) then 
		...
		local mod = enemy:FindModifierByName("modifier_sven_storm_bolt_custom_legendary_stack")

		if mod then 
			damage = damage  + mod:GetStackCount()*self:GetAbility().legendary_stack_damage
		end



		SendOverheadEventMessage(enemy, 4, enemy,  self:GetAbility():GetBoltDamage()*damage,nil)

		ApplyDamage(  { victim = enemy, attacker = self:GetCaster(), damage = self:GetAbility():GetBoltDamage()*damage, ability = self:GetAbility(), damage_type = damage_type, })
		
		if enemy:IsRealHero() and self:GetCaster():GetQuest() == "Sven.Quest_5" then 
			self:GetCaster():UpdateQuest(self:GetAbility().legendary_stun)
		end

		if enemy:IsRealHero() or enemy:IsIllusion() then 
			enemy:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_stunned", {duration = self:GetAbility().legendary_stun})
		end

		enemy:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_sven_storm_bolt_custom_legendary_stack", {duration = self:GetAbility().legendary_stack})
...
if mod and not params.target:IsMagicImmune() then 
	...
	SendOverheadEventMessage(enemy, 4, enemy,  self:GetAbility():GetBoltDamage()*self:GetAbility().proc_damage ,nil)

	if enemy:IsRealHero() and self:GetCaster():GetQuest() == "Sven.Quest_5" then 
		self:GetCaster():UpdateQuest(self:GetAbility().legendary_stun)
	end

	ApplyDamage(  { victim = enemy, attacker = self:GetCaster(), damage = self:GetAbility():GetBoltDamage()*self:GetAbility().proc_damage, ability = self:GetAbility(), damage_type = damage_type, })
	enemy:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_stunned", {duration = self:GetAbility().legendary_stun})
...
self:GetParent():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_sven_storm_bolt_custom_silence", {duration = (1 - self:GetParent():GetStatusResistance())*self:GetAbility().silence_duration})
end
