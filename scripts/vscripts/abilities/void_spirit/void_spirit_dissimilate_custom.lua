...
void_spirit_dissimilate_custom.damage_inc = {0.4, 0.6, 0.8}

void_spirit_dissimilate_custom.cd = {-1, -2, -3}
...
if self:GetCaster():HasModifier("modifier_void_astral_2") then 
  upgrade_cooldown = self.cd[self:GetCaster():GetUpgradeStack("modifier_void_astral_2")]
end
...
if not IsServer() then return end
	...
	if self:GetParent():HasModifier("modifier_void_astral_1") then 
		damage = damage +  self:GetAbility().damage_inc[self:GetParent():GetUpgradeStack("modifier_void_astral_1")]*self:GetCaster():GetIntellect()
	end
