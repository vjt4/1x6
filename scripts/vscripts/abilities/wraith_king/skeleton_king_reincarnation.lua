...
LinkLuaModifier("modifier_skeleton_king_reincarnation_custom_no_mana", "abilities/wraith_king/skeleton_king_reincarnation", LUA_MODIFIER_MOTION_NONE)
...
skeleton_king_reincarnation_custom.silence_delay = -1

skeleton_king_reincarnation_custom.damage_init = 3
skeleton_king_reincarnation_custom.damage_inc = 3

skeleton_king_reincarnation_custom.cd_inc = {-10, -15, -20}
...
skeleton_king_reincarnation_custom.aura_radius = 500
skeleton_king_reincarnation_custom.aura_interval = 1
skeleton_king_reincarnation_custom.aura_damage = {0.03, 0.05}
skeleton_king_reincarnation_custom.aura_heal = 0.7
skeleton_king_reincarnation_custom.aura_creep = 0.33
...
skeleton_king_reincarnation_custom.active_damage = 20

skeleton_king_reincarnation_custom.legendary_duration = 6
skeleton_king_reincarnation_custom.legendary_health = 0.5
skeleton_king_reincarnation_custom.legendary_start = 0.5
skeleton_king_reincarnation_custom.legendary_move = 15
...
function skeleton_king_reincarnation_custom:GetCooldown(level)
    ...
    if self:GetCaster():HasModifier("modifier_skeleton_reincarnation_3") then 
        k = self.cd_inc[self:GetCaster():GetUpgradeStack("modifier_skeleton_reincarnation_3")]
    end
...
if self:GetCaster():GetHealthPercent() < self.active_health then 
    CustomGameEventManager:Send_ServerToPlayer(PlayerResource:GetPlayer(self:GetCaster():GetPlayerOwnerID()), "CreateIngameErrorMessage", {message = "#reinc_health"})
    return false
end
    return true
end
...
self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_skeleton_king_reincarnation_custom_no_mana", {})
...
function skeleton_king_reincarnation_custom:GetManaCost(level)
    if self:GetCaster():HasShard() or self:GetCaster():HasModifier("modifier_skeleton_king_reincarnation_custom_no_mana") then
        return 0
    end
...
function modifier_skeleton_king_reincarnation_custom:ReincarnateTime()
    if IsServer() then
        ...
        if self:GetCaster():HasModifier("modifier_skeleton_reincarnation_5") then 
            bonus = self:GetAbility().silence_delay
        end
...
modifier_skeleton_king_reincarnation_custom_no_mana = class({})
function modifier_skeleton_king_reincarnation_custom_no_mana:IsHidden() return true end
function modifier_skeleton_king_reincarnation_custom_no_mana:IsPurgable() return false end
...
local damage = self:GetParent():GetHealth()*self:GetAbility().aura_damage[self:GetCaster():GetUpgradeStack("modifier_skeleton_reincarnation_4")]*self:GetAbility().aura_interval
local k = 1

if self:GetParent():IsCreep() then 
    k = self:GetAbility().aura_creep
end

damage = damage*k

if self:GetAbility():GetCooldownTimeRemaining() > 0 and not self:GetParent():IsIllusion() then 
    my_game:GenericHeal(self:GetCaster(), damage*self:GetAbility().aura_heal, self:GetAbility())
end
