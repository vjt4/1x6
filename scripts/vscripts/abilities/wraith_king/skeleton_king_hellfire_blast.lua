...
skeleton_king_hellfire_blast_custom.damage_inc = {0.2, 0.3, 0.4}
skeleton_king_hellfire_blast_custom.damage_inc_burn = {0.1, 0.15, 0.2}

skeleton_king_hellfire_blast_custom.stun_init = 0.1
skeleton_king_hellfire_blast_custom.stun_inc = 0.1
skeleton_king_hellfire_blast_custom.stun_range = {100, 150, 200}
...
skeleton_king_hellfire_blast_custom.legendary_damage = 0.5
...
if self:GetCaster():HasModifier("modifier_skeleton_blast_1") then 
  upgrade = self.stun_range[self:GetCaster():GetUpgradeStack("modifier_skeleton_blast_1")]
end
...
if not IsServer() then return end
    if target ~= nil and ( not target:IsMagicImmune() ) and ( not target:TriggerSpellAbsorb( self ) ) then
        ...
        if self:GetCaster():HasModifier("modifier_skeleton_blast_3") then 
            stun_damage = stun_damage + self:GetCaster():GetAverageTrueAttackDamage(nil)*(self.damage_inc[self:GetCaster():GetUpgradeStack("modifier_skeleton_blast_3")])
        end
        ...
        for _, skelet in pairs(all_skeletons) do
            if skelet:GetUnitName() == "npc_dota_wraith_king_skeleton_warrior_custom" then
                ...
                if modifier and not self:GetCaster():HasModifier("modifier_skeleton_vampiric_legendary") then
...
if not IsServer() then return end
    ...
    if self:GetCaster():HasModifier("modifier_skeleton_blast_3") then 
        damage = damage + self:GetCaster():GetAverageTrueAttackDamage(nil)*(self:GetAbility().damage_inc_burn[self:GetCaster():GetUpgradeStack("modifier_skeleton_blast_3")])
    end
