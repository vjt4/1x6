...
skeleton_king_mortal_strike_custom.cleave_slow = {-15, -20, -25}
skeleton_king_mortal_strike_custom.cleave_slow_duration = 2
...
skeleton_king_mortal_strike_custom.legendary_stack_duration = 7
skeleton_king_mortal_strike_custom.legendary_stun = 1

skeleton_king_mortal_strike_custom.stack_duration = 3
skeleton_king_mortal_strike_custom.stack_init = 0.05
skeleton_king_mortal_strike_custom.stack_inc = 0.1

skeleton_king_mortal_strike_custom.bkb_duration = 2
skeleton_king_mortal_strike_custom.bkb_cd = 12
skeleton_king_mortal_strike_custom.bkb_speed = 50
...
if self.record and self.record == params.record then
    ...
    and (params.target:IsCreep() or params.target:IsHero()) then    
        ...
        self:GetParent():AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_skeleton_king_mortal_strike_bkb_cd", {duration = self:GetAbility().bkb_cd})
    end 


    if self:GetParent():HasModifier("modifier_skeleton_strike_2") then 
        params.target:AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_skeleton_king_mortal_strike_slow", {duration = self:GetAbility().cleave_slow_duration*(1 - params.target:GetStatusResistance())})
...
return self:GetAbility().cleave_slow[self:GetCaster():GetUpgradeStack("modifier_skeleton_strike_2")]
end
