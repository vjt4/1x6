...
custom_pudge_dismember.blood_str = {6, 10}
...
if self:GetCaster():HasModifier("modifier_pudge_dismember_5") then 
	self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_custom_pudge_dismember_bkb", {duration = self.bkb_duration})
end
...
ParticleManager:SetParticleControlEnt(self.particle,0,self.target,PATTACH_POINT_FOLLOW,"attach_hitlock",Vector(0,0,0),false )
ParticleManager:SetParticleControlEnt(self.particle,1,self:GetCaster(),PATTACH_POINT_FOLLOW,"attach_hitlock",Vector(0,0,0),false)
