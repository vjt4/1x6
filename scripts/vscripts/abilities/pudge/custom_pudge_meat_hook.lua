...
custom_pudge_meat_hook.speed = {50,75,100}
custom_pudge_meat_hook.speed_duration = 3


custom_pudge_meat_hook.range_bonus = 300
custom_pudge_meat_hook.range_speed = 0.25
custom_pudge_meat_hook.range_radius = 2000
custom_pudge_meat_hook.range_health = 50


custom_pudge_meat_hook.blood_duration = 5
custom_pudge_meat_hook.blood_heal = {8, 12, 16}

custom_pudge_meat_hook.triple_angle = 5
custom_pudge_meat_hook.triple_count = 2 
custom_pudge_meat_hook.triple_cd = 1
custom_pudge_meat_hook.legendary_mana = 70

custom_pudge_meat_hook.attack_damage = {0.3, 0.45, 0.6}

custom_pudge_meat_hook.reduce_heal = -40
custom_pudge_meat_hook.reduce_duration = 1.5

custom_pudge_meat_hook.attacks_dist = 300
custom_pudge_meat_hook.attacks_damage = {-60, -30}
...
if not IsServer() then return end
    ...
    if self.hooks[projectileIndex].bRetracting == false then
        ...
        if target ~= nil then
            ...
            if not target:HasModifier("modifier_waveupgrade_boss") then 
            ...
            if target:GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then
                ...
                if self:GetCaster():HasModifier("modifier_pudge_hook_1") then 
                    damage = damage + self:GetCaster():GetAverageTrueAttackDamage(nil)*self.attack_damage[self:GetCaster():GetUpgradeStack("modifier_pudge_hook_1")]
                end   
                ...
                if self:GetCaster():HasModifier("modifier_pudge_hook_3") then 
                    self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_custom_pudge_meat_hook_blood", {duration = self.blood_duration})
                end
...
if not IsServer() then return end
    ...
    if self:GetParent():GetUnitName() ~= "npc_dota_companion" and self:GetCaster():HasModifier("modifier_pudge_hook_2") then 
        self:GetCaster():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_custom_pudge_meat_hook_speed", {duration = self:GetAbility().speed_duration})
    end
...
if not IsServer() then return end
if self:GetCaster():HasModifier("modifier_pudge_hook_4") and self:GetParent():GetUnitName() ~= "npc_dota_companion"  then 
    self:GetParent():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_custom_pudge_meat_hook_attacks", {})
end
...
return self:GetAbility().speed[self:GetCaster():GetUpgradeStack("modifier_pudge_hook_2")]
end

function modifier_custom_pudge_meat_hook_speed:OnAttackLanded(params)
if not IsServer() then return end
if self:GetParent() ~= params.attacker then return end

self:SetDuration(self:GetAbility().speed_duration, true)
...
self.heal = self:GetAbility().blood_heal[self:GetParent():GetUpgradeStack("modifier_pudge_hook_3")]/self:GetRemainingTime()
if not IsServer() then return end
...
modifier_custom_pudge_meat_hook_attacks = class({})
function modifier_custom_pudge_meat_hook_attacks:IsHidden() return true end
function modifier_custom_pudge_meat_hook_attacks:IsPurgable() return false end
function modifier_custom_pudge_meat_hook_attacks:OnCreated()
if not IsServer() then return end

self.dist = self:GetAbility().attacks_dist
...
function modifier_custom_pudge_meat_hook_attacks:OnIntervalThink()
if not IsServer() then return end
if not self:GetCaster() or not self:GetCaster():IsAlive() then return end



local dist = (self.loc - self:GetParent():GetAbsOrigin()):Length2D()

self.dist = self.dist + dist
self.loc = self:GetParent():GetAbsOrigin()

if self.dist >= self:GetAbility().attacks_dist then 

    self.dist = 0
    self:GetCaster():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_custom_pudge_meat_hook_damage", {})
    self:GetCaster():PerformAttack(self:GetParent(), true, true, true, true, false, false, true)
    self:GetCaster():RemoveModifierByName("modifier_custom_pudge_meat_hook_damage")
end

end

modifier_custom_pudge_meat_hook_damage = class({})
function modifier_custom_pudge_meat_hook_damage:IsHidden() return true end
function modifier_custom_pudge_meat_hook_damage:IsPurgable() return false end
function modifier_custom_pudge_meat_hook_damage:DeclareFunctions()
return
{
    MODIFIER_PROPERTY_DAMAGEOUTGOING_PERCENTAGE
}
end
function modifier_custom_pudge_meat_hook_damage:GetModifierDamageOutgoing_Percentage()
if not IsServer() then return end
return self:GetAbility().attacks_damage[self:GetCaster():GetUpgradeStack("modifier_pudge_hook_4")]
end
...
if cd > self:GetAbility().triple_cd then 
    self:GetAbility():StartCooldown(cd - self:GetAbility().triple_cd)
end
