...
custom_puck_dream_coil.resist_magic_init = -0.5
custom_puck_dream_coil.resist_magic_inc = -0.5
...
custom_puck_dream_coil.knockback_damage = 1.2
...
function modifier_custom_puck_dream_coil:OnIntervalThink()
	...
	if self:GetCaster():IsAlive() and self.counter >= self.rapid_fire_interval and self:GetAbility() and self:GetCaster():HasModifier("modifier_puck_coil_attacks") then
		...
		if (self:GetCaster():GetAbsOrigin() - self.coil_thinker_location):Length2D() <= self.rapid_fire_max_distance and not self:GetParent():IsAttackImmune()
			and self:GetParent():GetUnitName() ~= "npc_teleport" and not self:GetParent():HasModifier("modifier_monkey_king_wukongs_command_custom_soldier") then 
