...
mars_gods_rebuke_custom.cd_inc = {-1, -1.5, -2}


mars_gods_rebuke_custom.crit_inc = {30, 50, 70}

mars_gods_rebuke_custom.damage_heal = {0.3, 0.4, 0.5}
mars_gods_rebuke_custom.damage_heal_creeps = 0.33

mars_gods_rebuke_custom.stack_duration = 5
mars_gods_rebuke_custom.stack_max = 4
mars_gods_rebuke_custom.stack_damage = {30, 50}
mars_gods_rebuke_custom.stack_stun = {0.8, 1.2}

mars_gods_rebuke_custom.armor_reduction = 0.3
mars_gods_rebuke_custom.armor_duration = 5
...
mars_gods_rebuke_custom.attacks_speed = 200
mars_gods_rebuke_custom.attacks_range = 200
mars_gods_rebuke_custom.attacks_max = {2,3}
mars_gods_rebuke_custom.attacks_knock = 100
mars_gods_rebuke_custom.attacks_duration = 4
...
if self:GetCaster():HasModifier("modifier_mars_rebuke_1") then  
  upgrade_cooldown = self.cd_inc[self:GetCaster():GetUpgradeStack("modifier_mars_rebuke_1")]
end 
...
function mars_gods_rebuke_custom:GetCastPoint()
...
function mars_gods_rebuke_custom:Strike(point)
  ...
  local armor = self:GetCaster():GetPhysicalArmorValue(false)*self.armor_reduction*-1

  if self:GetCaster():HasModifier("modifier_mars_rebuke_5") then 
    self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_mars_gods_rebuke_custom_armor", {duration = self.armor_duration})
  end
  ...
  for _,enemy in pairs(enemies) do
    ...
    if enemy:GetUnitName() ~= "npc_teleport" and enemy:GetUnitName() ~= "modifier_monkey_king_wukongs_command_custom_soldier" then  
      ...
      if angle_diff<=angle then
        ...
        if self:GetCaster():HasModifier("modifier_mars_rebuke_5") then 
          enemy:AddNewModifier(caster, self, "modifier_generic_armor_reduction", {duration = self.armor_duration, armor_reduction = armor})
        end

        if self:GetCaster():HasModifier("modifier_mars_arena_of_blood_custom_projectile_aura") and not enemy:IsMagicImmune() and self:GetCaster():FindAbilityByName("mars_arena_of_blood_custom") and self:GetCaster():HasModifier("modifier_mars_arena_6") then 
          enemy:AddNewModifier(self:GetCaster(), self, "modifier_generic_silence", {duration = (1 - enemy:GetStatusResistance())*self:GetCaster():FindAbilityByName("mars_arena_of_blood_custom").spells_silence})
        end
  ...
  local ability = self:GetCaster():FindAbilityByName("mars_avatar_custom")
  if false and ability and ability:GetCooldownTimeRemaining() > 0 then 
    local cd = ability:GetCooldownTimeRemaining()

    ability:EndCooldown()
    if cd > ability:GetSpecialValueFor("self_cd") then 
      ability:StartCooldown(cd - ability:GetSpecialValueFor("self_cd"))
    end
  end

  if self:GetCaster():HasModifier("modifier_mars_rebuke_4") then 
    self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_mars_gods_rebuke_custom_attacks", {duration = self.attacks_duration})
  end
  buff:Destroy()
...
if self:GetParent():HasModifier("modifier_mars_rebuke_2") then 
  self.bonus_crit = self.bonus_crit + self:GetAbility().crit_inc[self:GetCaster():GetUpgradeStack("modifier_mars_rebuke_2")]
end
...
self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_mars_gods_rebuke_custom_legendary", {duration = self:GetSpecialValueFor("duration")})
...
self.cd_reduction = self:GetAbility():GetSpecialValueFor("shield_cd")
...
function modifier_mars_gods_rebuke_custom_legendary:OnStackCountChanged(iStackCount)
if self:GetStackCount() == 0 then return end

if self:GetStackCount() == 1 then 

  self.effect_cast = ParticleManager:CreateParticle( "particles/ogre_count.vpcf", PATTACH_OVERHEAD_FOLLOW, self:GetParent() )
...
if cd > self.cd_reduction then 
  ability:StartCooldown(cd - self.cd_reduction)
end
...
self:SetStackCount(self:GetAbility().attacks_max[self:GetCaster():GetUpgradeStack("modifier_mars_rebuke_4")])
end
...
if not params.target:HasModifier("modifier_mars_spear_custom_debuff") and (params.target:IsHero() or params.target:IsCreep())
  ...
  params.target:AddNewModifier(caster, self, "modifier_generic_knockback",
    {
      duration = 0.2,
      distance = math.max(0, math.min(self:GetAbility().attacks_knock, distance - 150)),
      height = 0,
      direction_x = dir.x,
      direction_y = dir.y,
    }
