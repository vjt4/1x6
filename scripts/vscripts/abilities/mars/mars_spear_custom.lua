...
mars_spear_custom.damage_inc = {50, 100, 150}

mars_spear_custom.range_cast = {100, 150, 200}
mars_spear_custom.range_stun = {0.2, 0.3, 0.4}

mars_spear_custom.legendary_duration = 2
mars_spear_custom.legendary_speed = 70
mars_spear_custom.legendary_damage = 130
mars_spear_custom.legendary_turn_speed = 120

mars_spear_custom.double_delay = 2
mars_spear_custom.double_damage = 0.25
...
if self:GetCaster():HasModifier("modifier_mars_spear_2") then 
  bonus = self.range_cast[self:GetCaster():GetUpgradeStack("modifier_mars_spear_2")]
end
...
if self:GetCaster():HasModifier("modifier_mars_spear_1") then  
  --upgrade_cooldown = self.cd_inc[self:GetCaster():GetUpgradeStack("modifier_mars_spear_1")]
end 
...
function mars_spear_custom:LaunchSpear(origin, point, k, init_launch, override_damage)
	...
	if self:GetCaster():HasModifier("modifier_mars_spear_2") then 
  		projectile_distance = projectile_distance + self.range_cast[self:GetCaster():GetUpgradeStack("modifier_mars_spear_2")]
	end
	...
	if init_launch == 0 then 
		projectile_distance = math.min(projectile_distance, (origin - point):Length2D())
	end
	...
	if self:GetCaster():HasModifier("modifier_mars_spear_3") then 
		damage = damage + self.damage_inc[self:GetCaster():GetUpgradeStack("modifier_mars_spear_3")]
	end
...
function modifier_mars_spear_custom_legendary:OnCreated( kv )
	...
	if self:GetCaster():HasModifier("modifier_mars_spear_2") then 
  		self.distance = self.distance + self:GetAbility().range_cast[self:GetCaster():GetUpgradeStack("modifier_mars_spear_2")]
	end
...
if not self.effect_cast then return end
	local target_pos = self.origin + self:GetParent():GetForwardVector() * self.distance
...
if caster:HasModifier("modifier_mars_spear_5") and ability.spear_array[projectileID].hit == false and self[projectileID].init_launch == 1 then 
	local cd = ability:GetCooldownTimeRemaining() 
	ability:EndCooldown()
	ability:StartCooldown(cd*(1 - ability.stun_cd/100))
end
...
function mars_spear_custom:Pinned( iProjectileHandle )
	...
	if self:GetCaster():HasModifier("modifier_mars_arena_of_blood_custom_projectile_aura") and self:GetCaster():HasModifier("modifier_mars_arena_6") and self:GetCaster():FindAbilityByName("mars_arena_of_blood_custom") then 
   
		data.unit:AddNewModifier(self:GetCaster(), self, "modifier_mars_spear_custom_incoming", {duration = self:GetCaster():FindAbilityByName("mars_arena_of_blood_custom").spells_damage_duration})
    end

	data.unit:AddNewModifier( caster, ability, "modifier_mars_spear_custom_debuff", { duration =  duration_stun} )


	if data.unit:IsRealHero() then 
...
function modifier_mars_spear_custom_str:IsHidden() return false end
...
self.damage = self:GetAbility():GetSpecialValueFor("shard_dps")*self.interval*table.damage/100
self:StartIntervalThink(self.interval)
end
...
function modifier_mars_spear_custom_incoming:GetModifierIncomingDamage_Percentage() 
	return self:GetCaster():FindAbilityByName("mars_arena_of_blood_custom").spells_damage
end
