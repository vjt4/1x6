...
primal_beast_uproar_custom.speed = {40,60,80}
primal_beast_uproar_custom.speed_duration = 2

primal_beast_uproar_custom.stats_damage = {4,8,12}
primal_beast_uproar_custom.stats_speed = {2,3,4}

primal_beast_uproar_custom.duration_magic = 4
primal_beast_uproar_custom.duration_magic_max = 20
primal_beast_uproar_custom.duration_inc = 1

primal_beast_uproar_custom.root_duration = 1.5
primal_beast_uproar_custom.root_slow_duration = 2

primal_beast_uproar_custom.proc_speed = 300
primal_beast_uproar_custom.proc_crit = {130, 200}
...
primal_beast_uproar_custom.cast_heal = {0.02, 0.03, 0.04}
...
function primal_beast_uproar_custom:GetBehavior()
if self:GetCaster():HasModifier("modifier_primal_beast_uproar_6") then 
...
function primal_beast_blood_frenzy_custom:GetBehavior()
if self:GetCaster():HasModifier("modifier_primal_beast_uproar_6") then 
	return DOTA_ABILITY_BEHAVIOR_NO_TARGET + DOTA_ABILITY_BEHAVIOR_IMMEDIATE + DOTA_ABILITY_BEHAVIOR_IGNORE_PSEUDO_QUEUE
end
return DOTA_ABILITY_BEHAVIOR_NO_TARGET + DOTA_ABILITY_BEHAVIOR_IMMEDIATE
...
function primal_beast_uproar_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_primal_beast_uproar_6") then 
		...
		duration = duration + self.duration_inc
	...
	if self:GetCaster():HasModifier("modifier_primal_beast_uproar_1") then 
		self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_primal_beast_uproar_custom_speed", {duration = self.speed_duration})
	end

	local ult = self:GetCaster():FindAbilityByName("primal_beast_pulverize_custom")
	if ult and stack == ult.legendary_roar and self:GetCaster():HasModifier("modifier_primal_beast_pulverize_7") then 
	...
	for _,enemy in pairs(enemies) do
		if self:GetCaster():HasModifier("modifier_primal_beast_uproar_5") then 
			...
			enemy:AddNewModifier(self:GetCaster(), self, "modifier_primal_beast_uproar_custom_root", {duration = self.root_duration*(1 - enemy:GetStatusResistance()), stack = stack })
		else 
...
local bonus = 0
if self:GetCaster():HasModifier("modifier_primal_beast_uproar_6") then 
	bonus = math.min(self:GetAbility().duration_magic_max, self:GetAbility().duration_magic*self:GetStackCount())
end
...
return self:GetAbility().speed[self:GetCaster():GetUpgradeStack("modifier_primal_beast_uproar_1")]
end
...
self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_primal_beast_uproar_custom_legendary", {duration = self:GetSpecialValueFor("duration")})
end
...
local duration = (self:GetAbility():GetSpecialValueFor("slow_duration") + self:GetAbility().root_slow_duration)*(1 - self:GetParent():GetStatusResistance())
