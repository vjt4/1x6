...
primal_beast_trample_custom.duration = {0.5, 1, 1.5}
...
primal_beast_trample_custom.stack_damage = {2, 4}
...
if self:GetCaster():HasModifier("modifier_primal_beast_trample_3") then 
	duration = duration + self.duration[self:GetCaster():GetUpgradeStack("modifier_primal_beast_trample_3")]
end
...
if not IsServer() then return end
	...
	if ult and #enemies > 0 and self:GetCaster():HasModifier("modifier_primal_beast_pulverize_7") then 
		...
		if self.ult_count >= ult.legendary_trample then 
			self.ult_count = 0
	...
	for _,enemy in pairs(enemies) do
		...
		if enemy:IsCreep() then 
			current_damage = current_damage*self:GetAbility():GetSpecialValueFor("creeps_damage")/100
		end
...
self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_primal_beast_trample_charge", {duration = self:GetSpecialValueFor("duration")})
end
