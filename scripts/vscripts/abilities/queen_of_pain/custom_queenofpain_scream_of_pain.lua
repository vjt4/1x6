LinkLuaModifier("modifier_custom_scream_fear_cd", "abilities/queen_of_pain/custom_queenofpain_scream_of_pain", LUA_MODIFIER_MOTION_NONE)
...
custom_queenofpain_scream_of_pain.fear_cd = 12
custom_queenofpain_scream_of_pain.fear_duration = 1.2
...
custom_queenofpain_scream_of_pain.heal_init = 0.02
custom_queenofpain_scream_of_pain.heal_inc = 0.02
custom_queenofpain_scream_of_pain.heal_damage = 0.30
...
if not IsServer() then	return end		
		...
		if self:GetCaster():HasModifier("modifier_queen_Scream_shield") then 
			...
			bonus = (self:GetCaster():GetMaxHealth() - self:GetCaster():GetHealth()) *(self.heal_init + self.heal_inc*self:GetCaster():GetUpgradeStack("modifier_queen_Scream_shield"))
		...
		local fear = false
		if self:GetCaster():HasModifier("modifier_queen_Scream_fear")  and not self:GetCaster():HasModifier("modifier_custom_scream_fear_cd") then 
			fear = true
			self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_custom_scream_fear_cd", {duration = self.fear_cd})
		end
...
if not target:IsBuilding() and ExtraData.fear == 1 then 
	...
	target:AddNewModifier(self:GetCaster(), self, name , {duration = self.fear_duration*(1 - target:GetStatusResistance())})
...
modifier_custom_scream_fear_cd = class({})
function modifier_custom_scream_fear_cd:IsHidden() return false end
function modifier_custom_scream_fear_cd:IsPurgable() return false end
function modifier_custom_scream_fear_cd:RemoveOnDeath() return false end
function modifier_custom_scream_fear_cd:IsDebuff() return true end
function modifier_custom_scream_fear_cd:GetTexture() return "buffs/scream_fear" end
function modifier_custom_scream_fear_cd:OnCreated(table)
self.RemoveForDuel = true
end
