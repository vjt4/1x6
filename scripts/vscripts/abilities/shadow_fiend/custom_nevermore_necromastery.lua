...
LinkLuaModifier("modifier_custom_necromastery_kills_max", "abilities/shadow_fiend/custom_nevermore_necromastery", LUA_MODIFIER_MOTION_NONE)
...
custom_nevermore_necromastery.kills_tresh = 10
custom_nevermore_necromastery.kills_damage = 1

custom_nevermore_necromastery.legendary_duration = 6
custom_nevermore_necromastery.legendary_cd = 40
custom_nevermore_necromastery.legendary_bva = 1.3
custom_nevermore_necromastery.legendary_soul_duration = 12
...
custom_nevermore_necromastery.attack_max = 6
custom_nevermore_necromastery.attack_damage = {5, 8} 
...
if mod then 
	...
	if mod:GetStackCount() >= self:GetAbility().attack_max then 
		mod:Destroy()
		local damage = self:GetAbility().attack_damage[self:GetCaster():GetUpgradeStack("modifier_nevermore_souls_tempo")] *self:GetStackCount()
...
local stacks = self:GetStackCount()
local bonus = 0 
if self:GetCaster():HasModifier("modifier_custom_necromastery_kills_max") then 
	bonus = self:GetAbility().kills_damage
end
...
if not IsServer() then return end
		...
		if self.caster == attacker and self.caster ~= target then
			...
			if target:IsRealHero() and not target:IsReincarnating() then 
...
if not  IsServer() then return end
if self:GetStackCount() >= self:GetAbility().kills_tresh then return end
self:IncrementStackCount()

if self:GetStackCount() >= self:GetAbility().kills_tresh then 
	self:GetCaster():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_custom_necromastery_kills_max", {})
end
...
if self:GetParent():HasModifier("modifier_nevermore_souls_kills") then 
return self:GetAbility().kills_max*self:GetStackCount()
end
...
modifier_custom_necromastery_kills_max = class({})
function modifier_custom_necromastery_kills_max:IsHidden() return true end
function modifier_custom_necromastery_kills_max:IsPurgable() return false end
function modifier_custom_necromastery_kills_max:RemoveOnDeath() return false end
