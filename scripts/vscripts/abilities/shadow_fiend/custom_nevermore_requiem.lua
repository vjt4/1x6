...
function modifier_custom_reqiuem_debuff:OnCreated()
	...
	if IsServer() then
		...
		if self.scepter and self.parent:IsRealHero() then
			local requiem_screen_particle = ParticleManager:CreateParticleForPlayer(self.particle_black_screen, PATTACH_EYES_FOLLOW, self.parent, PlayerResource:GetPlayer(self.parent:GetPlayerOwnerID()))
