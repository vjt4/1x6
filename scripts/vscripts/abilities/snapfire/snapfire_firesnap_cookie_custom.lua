...
snapfire_firesnap_cookie_custom.heal_cast = {100, 150, 200}
snapfire_firesnap_cookie_custom.heal_after = {100, 150, 200}
snapfire_firesnap_cookie_custom.heal_after_duration = 4

snapfire_firesnap_cookie_custom.speed_attack = {40, 60, 80}
snapfire_firesnap_cookie_custom.speed_move = {10, 15, 20}
snapfire_firesnap_cookie_custom.speed_duration = 4

snapfire_firesnap_cookie_custom.legendary_radius = 150
snapfire_firesnap_cookie_custom.legendary_max = 6
snapfire_firesnap_cookie_custom.legendary_min_range = 200
snapfire_firesnap_cookie_custom.legendary_mana = 30
snapfire_firesnap_cookie_custom.legendary_distance = 300
snapfire_firesnap_cookie_custom.legendary_timer = 4
snapfire_firesnap_cookie_custom.legendary_cd = 3.5

snapfire_firesnap_cookie_custom.bombs_max = 3
snapfire_firesnap_cookie_custom.bombs_interval = 0.2
snapfire_firesnap_cookie_custom.bombs_distance = 270
snapfire_firesnap_cookie_custom.bombs_aoe = 320
snapfire_firesnap_cookie_custom.bombs_timer = 3
snapfire_firesnap_cookie_custom.bombs_slow = -80
snapfire_firesnap_cookie_custom.bombs_slow_duration = 1.5
snapfire_firesnap_cookie_custom.bombs_damage = {0.05, 0.08}
snapfire_firesnap_cookie_custom.bombs_creeps = 0.33

snapfire_firesnap_cookie_custom.damage_inc = {4, 6, 8}
...
function snapfire_firesnap_cookie_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_snapfire_cookie_3") then 
		my_game:GenericHeal(self:GetCaster(), self.heal_cast[self:GetCaster():GetUpgradeStack("modifier_snapfire_cookie_3")], self)

		self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_snapfire_firesnap_cookie_custom_heal", {duration = self.heal_after_duration})
	end
	...
	distance = math.min(max_dist, math.max(self.legendary_min_range, vec:Length2D()))
	height = distance*0.55


	if ulti and ulti:GetLevel() > 0 and self:GetCaster():HasShard() then 
		ulti:LauchBall(self:GetCaster():GetAbsOrigin() + self:GetCaster():GetForwardVector()*distance, 0.7)
	end
...
self.heal = (self:GetAbility().heal_after[self:GetCaster():GetUpgradeStack("modifier_snapfire_cookie_3")])/self:GetRemainingTime()
...
return self:GetAbility().heal_after[self:GetCaster():GetUpgradeStack("modifier_snapfire_cookie_3")]/self:GetAbility().heal_after_duration
end
...
self:SetStackCount(self:GetAbility().bombs_max)

self:OnIntervalThink()
self:StartIntervalThink(self:GetAbility().bombs_interval)
end
...
local r = self:GetAbility().bombs_distance
...
self.timer = self:GetAbility().bombs_timer*2
self.t = 0
end
...
self:GetParent():SetRenderColor(255, 255 * (self:GetAbility().bombs_timer - self:GetElapsedTime() / self:GetAbility().bombs_timer ), 255 * (self:GetAbility().bombs_timer - self:GetElapsedTime() / self:GetAbility().bombs_timer ))
...
local enemies = FindUnitsInRadius( self:GetCaster():GetTeamNumber(), self:GetParent():GetOrigin(), nil, self:GetAbility().bombs_aoe, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, 0, 0, false)
for _,enemy in pairs(enemies) do

	local damage = self:GetAbility().bombs_damage[self:GetCaster():GetUpgradeStack("modifier_snapfire_cookie_4")]*enemy:GetMaxHealth()

	if enemy:IsCreep() then 
		damage = damage*self:GetAbility().bombs_creeps
	end
	...
	enemy:AddNewModifier( self:GetCaster(), self:GetAbility(), "modifier_snapfire_firesnap_cookie_custom_bomb_slow", { duration = (1 - enemy:GetStatusResistance())*self:GetAbility().bombs_slow_duration} )
end
...
function modifier_snapfire_firesnap_cookie_custom_bomb_slow:IsHidden() return false end
...
self.slow = self:GetAbility().bombs_slow
end
...
CustomGameEventManager:Send_ServerToPlayer(PlayerResource:GetPlayer(self:GetParent():GetPlayerOwnerID()), 'snapfire_cookie_change',  { max = self:GetAbility().legendary_max, current = self:GetRemainingTime(), active = 0})
...
CustomGameEventManager:Send_ServerToPlayer(PlayerResource:GetPlayer(self:GetParent():GetPlayerOwnerID()), 'snapfire_cookie_change',  { max = self:GetAbility().legendary_max, current = 0, active = 1})
...
self:StartIntervalThink(self:GetAbility().legendary_timer)
...
if self:GetStackCount() >= self:GetAbility().legendary_max then 
	...
	local point = target:GetAbsOrigin() + RandomVector(self:GetAbility().legendary_distance)

	self:GetParent():AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_snapfire_firesnap_cookie_custom_legendary_cd", {duration = self:GetAbility().legendary_cd})
...
CustomGameEventManager:Send_ServerToPlayer(PlayerResource:GetPlayer(self:GetParent():GetPlayerOwnerID()), 'snapfire_cookie_change',  { max = self:GetAbility().legendary_max, current = self:GetStackCount(), active = 1})
