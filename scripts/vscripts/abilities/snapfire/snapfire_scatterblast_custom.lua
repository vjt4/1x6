...
LinkLuaModifier( "modifier_snapfire_scatterblast_custom_timer", "abilities/snapfire/snapfire_scatterblast_custom", LUA_MODIFIER_MOTION_NONE )
...
snapfire_scatterblast_custom.knock_range = 230
snapfire_scatterblast_custom.knock_disarm = 1.5
snapfire_scatterblast_custom.knock_duration = 0.2
snapfire_scatterblast_custom.knock_cd = 5

snapfire_scatterblast_custom.legendary_shredder = 6
snapfire_scatterblast_custom.legendary_kisses = 1.5
snapfire_scatterblast_custom.legendary_cd = 0.6

snapfire_scatterblast_custom.stack_duration = 12
snapfire_scatterblast_custom.stack_damage = {30, 50}
snapfire_scatterblast_custom.stack_anim = 0.1
snapfire_scatterblast_custom.stack_max = 4

snapfire_scatterblast_custom.cd_inc = {-1, -2, -3}
...
return self.BaseClass.GetCastPoint(self) - self:GetCaster():GetUpgradeStack("modifier_snapfire_scatterblast_custom_stack")*self.stack_anim
end
...
if not target then 
    ...
    if self:GetCaster():HasModifier("modifier_snapfire_scatterblast_custom_stack") then 
        damage = damage + self.stack_damage[self:GetCaster():GetUpgradeStack("modifier_snapfire_scatter_4")]*self:GetCaster():GetUpgradeStack("modifier_snapfire_scatterblast_custom_stack")
    end
    ...
    if length <= point_blank_range then 
        ...
        if  self.array[#self.array] ~= 2 then 
            ...
            if self:GetCaster():HasModifier("modifier_snapfire_scatter_7") then 
                ...
                self:EndCooldown()
                self:StartCooldown(cd*(1 - self.legendary_cd))
            end

            if self:GetCaster():HasModifier("modifier_snapfire_scatter_4") then 
                self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_snapfire_scatterblast_custom_stack" , {duration = self.stack_duration})
            end
        end
        ...
        if self:GetCaster():HasModifier("modifier_snapfire_scatter_5") and not target:HasModifier("modifier_snapfire_scatterblast_custom_disarm_cd") then 
            ...
            local point = target:GetAbsOrigin() + dir*self.knock_range

            local distance = self.knock_range
            local speed = self.knock_range/self.knock_duration
            local height = 30

            target:AddNewModifier(self:GetCaster(), self, "modifier_snapfire_scatterblast_custom_disarm", {duration = self.knock_duration + (1 - target:GetStatusResistance())*self.knock_disarm})
             
            target:AddNewModifier(self:GetCaster(), self, "modifier_snapfire_scatterblast_custom_disarm_cd", {duration = self.knock_cd})
...
function modifier_snapfire_scatterblast_custom_stack:OnTooltip()
    return self:GetAbility().stack_damage[self:GetCaster():GetUpgradeStack("modifier_snapfire_scatter_4")]*self:GetStackCount()
end
...
modifier_snapfire_scatterblast_custom_timer = class({})
function modifier_snapfire_scatterblast_custom_timer:IsHidden() return true end
function modifier_snapfire_scatterblast_custom_timer:IsPurgable() return false end
function modifier_snapfire_scatterblast_custom_timer:RemoveOnDeath() return false end
function modifier_snapfire_scatterblast_custom_timer:OnCreated(table)
if not IsServer() then return end
...
function modifier_snapfire_scatterblast_custom_timer:OnIntervalThink()
if not IsServer() then return end

local mod = self:GetParent():FindModifierByName("modifier_snapfire_scatterblast_custom_stack")

local stack = 0
local max = self:GetAbility().stack_max
local active = 0
...
--CustomGameEventManager:Send_ServerToPlayer(PlayerResource:GetPlayer(self:GetParent():GetPlayerOwnerID()), 'snapfire_scatter_change',  { max = max, stack = stack, active = active})
