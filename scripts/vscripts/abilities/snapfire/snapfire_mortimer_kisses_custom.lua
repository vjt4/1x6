...
snapfire_mortimer_kisses_custom.cd_inc = {-20, -30, -40}

snapfire_mortimer_kisses_custom.max_inc = {2, 3, 4}

snapfire_mortimer_kisses_custom.heal = {0.2, 0.3, 0.4}
snapfire_mortimer_kisses_custom.heal_reduce = {-15, -20, -25}
snapfire_mortimer_kisses_custom.heal_creeps = 0.25

snapfire_mortimer_kisses_custom.bkb_duration = 3
snapfire_mortimer_kisses_custom.bkb_turn = -70

snapfire_mortimer_kisses_custom.reduce_damage = -60
snapfire_mortimer_kisses_custom.reduce_bkb = 2


snapfire_mortimer_kisses_custom.napalm_radius = 300
snapfire_mortimer_kisses_custom.napalm_duration = 8
snapfire_mortimer_kisses_custom.napalm_slow = {-2, -4}
snapfire_mortimer_kisses_custom.napalm_max = 10
snapfire_mortimer_kisses_custom.napalm_damage = {25, 40}
snapfire_mortimer_kisses_custom.napalm_attack = 4
snapfire_mortimer_kisses_custom.napalm_range = 800
...
if self:GetCaster():HasModifier("modifier_snapfire_kisses_1") then 
  upgrade_cooldown = self.cd_inc[self:GetCaster():GetUpgradeStack("modifier_snapfire_kisses_1")]
end
...
if self:GetCaster():HasModifier("modifier_snapfire_kisses_5") then 
    ...
    self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_generic_bkb", {duration = self.reduce_bkb})
end
...
function snapfire_mortimer_kisses_custom:LauchBall(location, speed)
...
ParticleManager:SetParticleControl(napalm_impact_particle, 1, Vector(self.napalm_radius, 0, 0))
...
local enemies = FindUnitsInRadius( self:GetCaster():GetTeamNumber(), point,  nil,  self.napalm_radius, DOTA_UNIT_TARGET_TEAM_ENEMY,   DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, 0,  0,  false   )

for _,enemy in pairs(enemies) do
    enemy:AddNewModifier(self:GetCaster(), self, "modifier_snapfire_mortimer_kisses_custom_napalm", {duration = self.napalm_duration})
end
...
if self:GetCaster():HasModifier("modifier_snapfire_kisses_6") then 
    duration = duration + self.bkb_duration
end
...
return self:GetAbility().reduce_damage
end
...
if not self:GetCaster():HasModifier("modifier_snapfire_kisses_6") then return end
    return self:GetAbility().bkb_turn
end

function modifier_snapfire_mortimer_kisses_custom_debuff:GetModifierMoveSpeedBonus_Percentage()
if self:GetParent():IsMagicImmune() and not self:GetCaster():HasModifier("modifier_snapfire_kisses_6") then return end
    return self.slow
end
...
if self:GetParent():IsMagicImmune() and not self:GetCaster():HasModifier("modifier_snapfire_kisses_6") then return end
    return self:GetAbility().heal_reduce[self:GetCaster():GetUpgradeStack("modifier_snapfire_kisses_3")]
end
...
if mod then 
    damage = damage + self:GetAbility().napalm_damage[self:GetCaster():GetUpgradeStack("modifier_snapfire_kisses_4")]*mod:GetStackCount()
end
...
function modifier_snapfire_mortimer_kisses_custom_thinker:GetAuraSearchFlags()
if self:GetCaster():HasModifier("modifier_snapfire_kisses_6") then 
...
if params.unit:IsCreep() then 
    k = self:GetAbility().heal_creeps
end

my_game:GenericHeal(self:GetCaster(), k*params.damage*self:GetAbility().heal[self:GetCaster():GetUpgradeStack("modifier_snapfire_kisses_3")], self:GetAbility())
...
if self:GetStackCount() >= self:GetAbility().napalm_attack then 
...
local units_heroes = FindUnitsInRadius( self:GetParent():GetTeamNumber(),  self:GetParent():GetAbsOrigin(),  nil, self:GetAbility().napalm_range,  DOTA_UNIT_TARGET_TEAM_ENEMY,  DOTA_UNIT_TARGET_HERO,  DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE,  FIND_CLOSEST,  false )
local units_creeps = FindUnitsInRadius( self:GetParent():GetTeamNumber(),  self:GetParent():GetAbsOrigin(),  nil, self:GetAbility().napalm_range,  DOTA_UNIT_TARGET_TEAM_ENEMY,  DOTA_UNIT_TARGET_BASIC,  DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE,  FIND_CLOSEST,  false )
...
if not IsServer() then return end
if self:GetStackCount() >= self:GetAbility().napalm_max then return end
...
return self:GetStackCount()*self:GetAbility().napalm_slow[self:GetCaster():GetUpgradeStack("modifier_snapfire_kisses_4")]
end


function modifier_snapfire_mortimer_kisses_custom_napalm:OnTooltip()
return self:GetStackCount()*self:GetAbility().napalm_damage[self:GetCaster():GetUpgradeStack("modifier_snapfire_kisses_4")]
end
...
self.max = math.floor( (self.main_ability:GetSpecialValueFor("projectile_count") +  bonus)*(self:GetAbility():GetSpecialValueFor("max")/100))
...
self.info = {
        ...
        ExtraData = 
        {
            damage = self:GetAbility():GetSpecialValueFor("damage")/100
        }
    }
...
if self:GetCaster():HasModifier("modifier_snapfire_kisses_6") then 
    flags = DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE + DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES
end
...
--local units_heroes = FindUnitsInRadius( self:GetParent():GetTeamNumber(),  self:GetParent():GetAbsOrigin(),  nil, 800,  DOTA_UNIT_TARGET_TEAM_ENEMY,  DOTA_UNIT_TARGET_HERO,  DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE,  FIND_CLOSEST,  false )
--local units_creeps = FindUnitsInRadius( self:GetParent():GetTeamNumber(),  self:GetParent():GetAbsOrigin(),  nil, 800,  DOTA_UNIT_TARGET_TEAM_ENEMY,  DOTA_UNIT_TARGET_BASIC,  DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE,  FIND_CLOSEST,  false )
...
self:StartIntervalThink(self.ability.legendary_kisses - 0.02)
end