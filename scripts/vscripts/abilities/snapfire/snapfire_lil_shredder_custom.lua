...
snapfire_lil_shredder_custom.attack_damage = {0.3, 0.5, 0.7}


snapfire_lil_shredder_custom.max_duration = {1, 2 ,3}
snapfire_lil_shredder_custom.max_attacks = {1, 2, 3}

snapfire_lil_shredder_custom.bash_armor = 3
snapfire_lil_shredder_custom.bash_stun = 0.2
snapfire_lil_shredder_custom.bash_chance = {20, 35}


snapfire_lil_shredder_custom.bonus_armor = {1, 1.5, 2}
snapfire_lil_shredder_custom.bonus_move = {4, 6, 8}
snapfire_lil_shredder_custom.bonus_duration = 8
...
snapfire_lil_shredder_custom.silence_count = 6
snapfire_lil_shredder_custom.silence_duration = 1.5
snapfire_lil_shredder_custom.silence_slow = -80
snapfire_lil_shredder_custom.silence_count_duration = 8
snapfire_lil_shredder_custom.silence_cd = -2

snapfire_lil_shredder_custom.legendary_max = 100
snapfire_lil_shredder_custom.legendary_attack = 20
snapfire_lil_shredder_custom.legendary_stun = 1.5
snapfire_lil_shredder_custom.legendary_decay = 40
...
if self:GetCaster():HasModifier("modifier_snapfire_shredder_2") then
  duration = duration + self.max_duration[self:GetCaster():GetUpgradeStack("modifier_snapfire_shredder_2")]
end 
...
function modifier_snapfire_lil_shredder_custom:OnCreated( kv )
  ...
  if self:GetParent():HasModifier("modifier_snapfire_shredder_1") then 
    self.damage = self.damage + self:GetAbility().attack_damage[self:GetCaster():GetUpgradeStack("modifier_snapfire_shredder_1")]*self:GetParent():GetAverageTrueAttackDamage(nil)
  end

  if self:GetParent():HasModifier("modifier_snapfire_shredder_2") then
    self.attacks = self.attacks + self:GetAbility().max_attacks[self:GetCaster():GetUpgradeStack("modifier_snapfire_shredder_2")]
  end 
...
self:SetStackCount(math.max(0, self:GetStackCount() - self:GetAbility().legendary_decay*0.1))
...
if self:GetStackCount() >= self:GetAbility().legendary_max then 
  ...
  self:GetParent():AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_stunned", {duration = ( 1 - self:GetParent():GetStatusResistance())*self:GetAbility().legendary_stun})
  self:Destroy()
end
...
CustomGameEventManager:Send_ServerToPlayer(PlayerResource:GetPlayer(self:GetParent():GetPlayerOwnerID()), 'snapfire_shredder_change',  {time = time, max = self:GetAbility().legendary_max, damage = self:GetStackCount()})
...
CustomGameEventManager:Send_ServerToPlayer(PlayerResource:GetPlayer(self:GetParent():GetPlayerOwnerID()), 'snapfire_shredder_change',  {time = 0, max = self:GetAbility().legendary_max, damage = self:GetStackCount()})
...
if not self:GetParent():HasModifier("modifier_snapfire_lil_shredder_custom_no_aoe") then 
  ...
  if self:GetCaster():HasModifier("modifier_snapfire_scatter_7") and self.scatter_ability and self.scatter_ability:GetLevel() > 0 then 
    ...
    if self.scatter_count >= self.scatter_ability.legendary_shredder then 
      self.scatter_count = 0
  ...
  if self:GetParent():HasModifier("modifier_snapfire_shredder_7") then 
    self:SetStackCount(self:GetStackCount() + self:GetAbility().legendary_attack)


  end

  if self:GetCaster():HasModifier("modifier_snapfire_shredder_3") then 
    self:GetCaster():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_snapfire_lil_shredder_custom_armor", {duration = self:GetAbility().bonus_duration})
    self:GetCaster():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_snapfire_lil_shredder_custom_armor_count", {duration = self:GetAbility().bonus_duration})
  end
...
if self:GetParent() ~= params.attacker then return end
  ...
  if self.records[params.record] then
    ...
    if self:GetParent():HasModifier("modifier_snapfire_shredder_6") then 
      params.target:AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_snapfire_lil_shredder_custom_silence_count", {duration = self:GetAbility().silence_count_duration})
    end
...
if not RollPseudoRandomPercentage(self:GetAbility().bash_chance[self:GetCaster():GetUpgradeStack('modifier_snapfire_shredder_4')] ,283,self:GetParent()) then return end
    

local duration = self:GetAbility():GetSpecialValueFor("armor_duration")

for i = 1, self:GetAbility().bash_armor do 
...
params.target:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_bashed", {duration = self:GetAbility().bash_stun})
...
return self:GetStackCount()*self:GetAbility().bonus_armor[self:GetCaster():GetUpgradeStack("modifier_snapfire_shredder_3")]
end


function modifier_snapfire_lil_shredder_custom_armor:GetModifierMoveSpeedBonus_Constant()
return self:GetStackCount()*self:GetAbility().bonus_move[self:GetCaster():GetUpgradeStack("modifier_snapfire_shredder_3")]
end
...
if self:GetStackCount() >= self:GetAbility().silence_count then 
  ...
  self:GetParent():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_snapfire_lil_shredder_custom_silence", {duration = (1 - self:GetParent():GetStatusResistance())*self:GetAbility().silence_duration})
  self:Destroy()
end
