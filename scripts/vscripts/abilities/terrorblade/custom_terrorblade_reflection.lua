...
custom_terrorblade_reflection.bonus_speed = {40, 60, 80}
custom_terrorblade_reflection.bonus_damage = {10, 15, 20}
...
custom_terrorblade_reflection.spells_cd = 2


custom_terrorblade_reflection.attack_heal = {20, 30, 40}
	
custom_terrorblade_reflection.slow_armor = -2
custom_terrorblade_reflection.slow_move = -5
custom_terrorblade_reflection.armor_duration = 3
custom_terrorblade_reflection.armor_max = {4, 7}

custom_terrorblade_reflection.legendary_duration = 2
custom_terrorblade_reflection.legendary_creeps = 2
...
if not IsServer() then return end
	...
	if caster:HasModifier("modifier_custom_terrorblade_reflection_legendary_saved") and self:GetAutoCastState() == true then 
		creeps = FindUnitsInRadius(caster:GetTeamNumber(), self:GetCursorPosition(), nil, self:GetSpecialValueFor("range"), DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_BASIC + DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_NOT_ILLUSIONS +  DOTA_UNIT_TARGET_FLAG_NO_INVIS, FIND_ANY_ORDER, false)
	end
	...
	if #creeps > 0 then 
		...
		for i,creep in pairs(creeps) do
			if not creep:IsHero() and (creep:IsCreep() or creep:IsConsideredHero()) then
				...
				if max == self.legendary_creeps then 
					break
				end
			end
		end
...
function modifier_custom_terrorblade_reflection_invulnerability:GetModifierAttackSpeedBonus_Constant()
local bonus = 0 
if self:GetCaster():HasModifier("modifier_terror_reflection_speed") then 
	bonus = self:GetAbility().bonus_speed[self:GetCaster():GetUpgradeStack("modifier_terror_reflection_speed")]
end

return bonus
end

function modifier_custom_terrorblade_reflection_invulnerability:GetModifierTotalDamageOutgoing_Percentage()
local bonus = 0 
if self:GetCaster():HasModifier("modifier_terror_reflection_speed") then 
	bonus = self:GetAbility().bonus_damage[self:GetCaster():GetUpgradeStack("modifier_terror_reflection_speed")]
end
...
if self:GetCaster():HasModifier("modifier_terror_reflection_slow") then
	my_game:GenericHeal(self:GetCaster(), self:GetAbility().attack_heal[self:GetCaster():GetUpgradeStack("modifier_terror_reflection_slow")], self:GetAbility())
end
...
for i = 0,self:GetParent():GetAbilityCount()-1 do
    ...
    if ContainsValue(a:GetBehavior(), DOTA_ABILITY_BEHAVIOR_UNIT_TARGET) 
        ...
        and not ContainsValue(a:GetBehavior(), DOTA_ABILITY_BEHAVIOR_AUTOCAST) and a:GetName() ~= "monkey_king_tree_dance_custom"  then
...
if not IsServer() then return end
if self:GetStackCount() >= self:GetAbility().armor_max[self:GetCaster():GetUpgradeStack("modifier_terror_reflection_silence")] then return end
self:IncrementStackCount()
end

function modifier_custom_terrorblade_reflection_armor:GetModifierPhysicalArmorBonus()
return self:GetStackCount()*(self:GetAbility().slow_armor)
end


function modifier_custom_terrorblade_reflection_armor:GetModifierMoveSpeedBonus_Percentage()
return self:GetStackCount()*(self:GetAbility().slow_move)
end
