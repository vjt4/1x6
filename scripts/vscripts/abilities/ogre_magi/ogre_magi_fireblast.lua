...
if not dont_stun then 
	...
	if self:GetCaster():GetQuest() == "Ogre.Quest_5" and target:IsRealHero() and not self:GetCaster():QuestCompleted() then 
		self:GetCaster():UpdateQuest(duration * ( 1 - target:GetStatusResistance() ))
	end
