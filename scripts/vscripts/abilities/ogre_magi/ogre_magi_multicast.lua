...
for i = 0, 5 do
    ...
    if current_item and current_item:GetName() ~= "item_aeon_disk" and current_item:GetName() ~= "item_midas_custom" and current_item:GetCooldownTimeRemaining() > 0 then  
        items[#items + 1] = i
    end
end
...
function modifier_ogre_magi_multicast_custom:OnAbilityFullyCast( params )
	...
	if self:GetParent():GetQuest() == "Ogre.Quest_8" and target and target:IsRealHero() and target:GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then 
...
function modifier_ogre_magi_multicast_custom_use:OnIntervalThink()
	...
	if self.no_target == 1 then 
		...
		if self.ability:IsItem() then
			...
			if self:GetParent():GetQuest() == "Ogre.Quest_8" and current_target and current_target:IsRealHero() then 
				self:GetParent():UpdateQuest(1)
			end
			...
			if #units > 0 then 
				...
				if name == "ogre_magi_fireblast_custom" or name == "ogre_magi_unrefined_fireblast_custom" then 
					if self:GetCaster():HasModifier("modifier_ogremagi_blast_7") then 
						...
						if self:GetParent():GetQuest() == "Ogre.Quest_8" and target and target:IsRealHero() then 
							self:GetParent():UpdateQuest(1)
						end
...
for itemSlot=0,5 do
	...
	if item and item:GetName() == name and #units > 0 then 
		...
		if bit.band( item:GetAbilityTargetTeam(), DOTA_UNIT_TARGET_TEAM_BOTH) ~= 0 or bit.band( item:GetAbilityTargetTeam(), DOTA_UNIT_TARGET_TEAM_ENEMY ) ~= 0 then
			...
			local current_target = units[RandomInt(1, #units)]
			self:GetParent():SetCursorCastTarget( current_target )
