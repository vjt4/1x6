...
antimage_mana_break_custom.burn_damage = {0.4,0.6,0.8}

antimage_mana_break_custom.heal = {10,20,30}
...
antimage_mana_break_custom.silence_cd = 10
antimage_mana_break_custom.silence_duration = 2

antimage_mana_break_custom.slow_reduction = -30
antimage_mana_break_custom.slow_mana = 50

antimage_mana_break_custom.legenday_duration = 8
antimage_mana_break_custom.legenday_damage = 1.5
antimage_mana_break_custom.legenday_slow_k = 2
antimage_mana_break_custom.legenday_cd = 16

antimage_mana_break_custom.mana_cost = {-15,-20,-25}
antimage_mana_break_custom.mana_cost_duration = 2

antimage_mana_break_custom.mana_burn = {1, 1.5, 2}
...
if target:IsMagicImmune() then return end
	...
	if self:GetParent():HasModifier("modifier_antimage_break_3") then 
		mana_per_hit_pct = mana_per_hit_pct + self:GetAbility().mana_burn[self:GetCaster():GetUpgradeStack("modifier_antimage_break_3")]
	end
	...
	if self:GetParent():IsIllusion() then
		...
		if self:GetParent():HasScepter() then 
			reduce_mana_full = reduce_mana_full + self:GetAbility():GetSpecialValueFor("scepter_bonus")
		end
	end
	...
	if self:GetParent():HasModifier("modifier_antimage_break_5") and not target:HasModifier("modifier_antimage_mana_break_custom_silence_cd") then 
		...
		target:AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_antimage_mana_break_custom_silence_cd", {duration = self:GetAbility().silence_cd})
	end
	...
	if self:GetParent():HasShard() and mana_burn > 2 and self:GetParent():IsRealHero() and not self:GetParent():HasModifier("modifier_tidehunter_anchor_smash_caster") then 
		...
		for _,unit in pairs(units) do 
			if unit ~= target then 
				ApplyDamage( { victim = unit, attacker = self:GetParent(), damage = mana_burn*self:GetAbility():GetSpecialValueFor("shard_damage")/100, damage_type = DAMAGE_TYPE_MAGICAL, ability = self:GetAbility()} )
			end
		end
