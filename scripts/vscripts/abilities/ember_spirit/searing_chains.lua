...
ember_spirit_searing_chains_custom.duration_inc = 0.2
ember_spirit_searing_chains_custom.radius_inc = {50, 75, 100}
...
ember_spirit_searing_chains_custom.legendary_damage = 0.06

ember_spirit_searing_chains_custom.heal_init = 0.1
ember_spirit_searing_chains_custom.heal_inc = 0.05

ember_spirit_searing_chains_custom.stun_duration = 0.5

ember_spirit_searing_chains_custom.reduction_heal = -30
ember_spirit_searing_chains_custom.reduction_damage = -30

ember_spirit_searing_chains_custom.resist_duration = 10
ember_spirit_searing_chains_custom.resist_magic = {-2, -4}
ember_spirit_searing_chains_custom.resist_chains = 2
ember_spirit_searing_chains_custom.resist_max = 6
...
if self:GetCaster():HasModifier("modifier_ember_chain_1") then 
  upgrade = self.radius_inc[self:GetCaster():GetUpgradeStack("modifier_ember_chain_1")]
end
...
if not IsServer() then return end
	...
	if self:GetCaster():HasModifier("modifier_ember_chain_1") then 
  		radius = radius + self.radius_inc[self:GetCaster():GetUpgradeStack("modifier_ember_chain_1")]
...
if self:GetCaster():HasModifier("modifier_ember_chain_4") then
	params.target:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_searing_chains_custom_resist", {duration = self:GetAbility().resist_duration})
end
...
local k = 1
if self:GetParent():HasModifier("modifier_searing_chains_custom_debuff") then 
	k = self:GetAbility().resist_chains
end

return self:GetStackCount()*(self:GetAbility().resist_magic[self:GetCaster():GetUpgradeStack("modifier_ember_chain_4")])*k
end