...
ember_spirit_sleight_of_fist_custom.attack_cd = {0.25, 0.4}
ember_spirit_sleight_of_fist_custom.attack_damage = {10, 20}
...
ember_spirit_sleight_of_fist_custom.evasion_move = 30
		...
		if #sleight_targets >= 1 then
			...
			Timers:CreateTimer(0, function()
				...
				if #sleight_targets >= current_count and caster:HasModifier("modifier_ember_spirit_sleight_of_fist_custom_caster") then
					previous_position = self:GetCaster():GetAbsOrigin()
					...
					if current_target and not current_target:IsNull() and not (current_target:IsInvisible() and not caster:CanEntityBeSeenByMyTeam(current_target)) and not current_target:IsAttackImmune() and current_target:IsAlive() then
...
return self:GetAbility().evasion_move
end
...
if cd > 0 then 
	...
	print(self:GetAbility():GetCooldownTimeRemaining())
end
