...
leshrac_split_earth_custom.range_distance = {100, 150, 200}
...
leshrac_split_earth_custom.heal_amount = {0.08, 0.12, 0.16}

leshrac_split_earth_custom.charge_speed = 1250

leshrac_split_earth_custom.legendary_distance = 700
leshrac_split_earth_custom.legendary_radius = 180
leshrac_split_earth_custom.legendary_aoe = 800
leshrac_split_earth_custom.legendary_stun = 0.6
leshrac_split_earth_custom.legendary_delay = 2
leshrac_split_earth_custom.legendary_damage = 600
leshrac_split_earth_custom.legendary_proc = 0.6
leshrac_split_earth_custom.legendary_cd = 3
leshrac_split_earth_custom.legendary_creeps = 0.25

leshrac_split_earth_custom.stack_max = 8
leshrac_split_earth_custom.stack_duration = 5
leshrac_split_earth_custom.stack_damage = {6, 10}
leshrac_split_earth_custom.stack_stun = {0.2, 0.3}
leshrac_split_earth_custom.stack_radius = 600
...
local upgrade = 0
if self:GetCaster():HasModifier("modifier_leshrac_earth_1") then 
  upgrade = self.range_distance[self:GetCaster():GetUpgradeStack("modifier_leshrac_earth_1")]
end
...
if mod then 
  damage = damage * (1 + self.stack_damage[self:GetCaster():GetUpgradeStack("modifier_leshrac_earth_4")]*mod:GetStackCount()/100)
end
...
local enemies_creeps = FindUnitsInRadius(self:GetCaster():GetTeamNumber(), self:GetCaster():GetOrigin(), nil, self.legendary_aoe, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_BASIC, 0, 0, false)
local enemies_heroes = FindUnitsInRadius(self:GetCaster():GetTeamNumber(), self:GetCaster():GetOrigin(), nil, self.legendary_aoe, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, 0, 0, false)
...
if enemy ~= nil then 
  ...
  point = self:GetCaster():GetAbsOrigin() + RandomVector(RandomInt(1, self.legendary_aoe))
end

CreateModifierThinker( self:GetCaster(), self, "modifier_leshrac_split_earth_custom_legendary_aoe", {duration = self.legendary_delay}, point, self:GetCaster():GetTeamNumber(), false)
end
...
for _,enemy in pairs(enemies) do
  ...
  if enemy:HasModifier("modifier_leshrac_split_earth_custom_legendary_stack") and enemy:FindModifierByName("modifier_leshrac_split_earth_custom_legendary_stack"):GetStackCount() >= self:GetAbility().stack_max then 
    duration = duration * (1 + self:GetAbility().stack_stun[self:GetCaster():GetUpgradeStack("modifier_leshrac_earth_4")])
  end
...
if self:GetCaster():HasModifier("modifier_leshrac_earth_3") and self.radius > (self:GetCaster():GetAbsOrigin() - self:GetParent():GetAbsOrigin()):Length2D() then 
  my_game:GenericHeal(self:GetCaster(), self:GetAbility().heal_amount[self:GetCaster():GetUpgradeStack("modifier_leshrac_earth_3")]*self:GetCaster():GetMaxHealth(), self:GetAbility())
...
local enemies = FindUnitsInRadius(self:GetCaster():GetTeamNumber(), self:GetParent():GetOrigin(), nil, self:GetAbility().stack_radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, 0, 0, false)

for _,enemy in pairs(enemies) do 
  enemy:AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_leshrac_split_earth_custom_legendary_stack", {duration = self:GetAbility().stack_duration})
end
...
params.target:AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_leshrac_split_earth_custom_legendary_stack", {duration = self:GetAbility().stack_duration})
...
if params.unit:IsCreep() then 
  k = self:GetAbility().legendary_creeps
end

self.damage_count = self.damage_count + params.damage*k

if self.damage_count >= self:GetAbility().legendary_damage then 
  self.damage_count = 0-- math.min(self.damage_count - self:GetAbility().legendary_damage, self:GetAbility().legendary_damage - 1)

  self:GetCaster():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_leshrac_split_earth_custom_legendary_cd", {duration = self:GetAbility().legendary_cd})
...
for _,enemy in pairs(enemies) do
    ...
    local duration = self:GetAbility().legendary_stun
    if enemy:HasModifier("modifier_leshrac_split_earth_custom_legendary_stack") and enemy:FindModifierByName("modifier_leshrac_split_earth_custom_legendary_stack"):GetStackCount() >= self:GetAbility().stack_max then 
      duration = duration * (1 + self:GetAbility().stack_stun[self:GetCaster():GetUpgradeStack("modifier_leshrac_earth_4")])
    end
    ...
    local knockbackProperties =
    {
        ...
        knockback_duration = self:GetAbility().legendary_stun,
        knockback_distance = 0,
        knockback_height = 150
    }

    ApplyDamage( { attacker = self:GetCaster(),victim = enemy,  damage = self:GetAbility():GetStunDamage(enemy)*self:GetAbility().legendary_proc, damage_type = DAMAGE_TYPE_MAGICAL, ability = self:GetAbility(), })
...
return self:GetAbility().stack_damage[self:GetCaster():GetUpgradeStack("modifier_leshrac_earth_4")]*self:GetStackCount()
end



function modifier_leshrac_split_earth_custom_legendary_stack:OnTooltip2()
return self:GetAbility().stack_stun[self:GetCaster():GetUpgradeStack("modifier_leshrac_earth_4")]
end