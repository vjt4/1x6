...
leshrac_lightning_storm_custom.damage_inc = {30, 45, 60}

leshrac_lightning_storm_custom.heal_damage = {0.3 , 0.45, 0.6}
leshrac_lightning_storm_custom.heal_creeps = 0.25

leshrac_lightning_storm_custom.legendary_radius = 200
leshrac_lightning_storm_custom.legendary_interval = 0.25
leshrac_lightning_storm_custom.legendary_max = 6
leshrac_lightning_storm_custom.legendary_waste_timer = 5
leshrac_lightning_storm_custom.legendary_waste_interval = 0.25
leshrac_lightning_storm_custom.legendary_damage = 0.4
leshrac_lightning_storm_custom.legendary_silence = 1

leshrac_lightning_storm_custom.attack_slow = 0.3
leshrac_lightning_storm_custom.attack_damage = 0.5
leshrac_lightning_storm_custom.attack_bounce = 1
leshrac_lightning_storm_custom.attack_chance = {20, 30}

leshrac_lightning_storm_custom.speed_duration = 5
...
leshrac_lightning_storm_custom.knockback_duration = 0.3
leshrac_lightning_storm_custom.knockback_distance_max = 400
leshrac_lightning_storm_custom.knockback_distance_min = 100
leshrac_lightning_storm_custom.knockback_radius = 300
leshrac_lightning_storm_custom.knockback_cd = 8

leshrac_lightning_storm_custom.bkb_range = 150
leshrac_lightning_storm_custom.bkb_duration = 0.2
...
if self:GetCaster():HasModifier("modifier_leshrac_storm_7") and legen_mod and legen_mod:GetStackCount() > 0 then  
  ...
  if legen_mod:GetStackCount() >= self.legendary_max then 
    target:AddNewModifier(self:GetCaster(), self, "modifier_generic_silence", {sound = "Sf.Raze_Silence", duration = (1 - target:GetStatusResistance())*self.legendary_silence})
  end
...
target:AddNewModifier(self:GetCaster(), self, "modifier_leshrac_lightning_storm_custom_slow", {duration = duration*(1 - target:GetStatusResistance())})
...
local enemies = FindUnitsInRadius(self:GetCaster():GetTeamNumber(), self:GetParent():GetAbsOrigin(), nil, self:GetAbility().legendary_radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, flag, 0, false )

for _,target in pairs(enemies) do 
  self:GetAbility():DealDamage(target, self:GetAbility().legendary_damage, 0)
end
...
self:StartIntervalThink(self:GetAbility().legendary_waste_timer)
...
self:StartIntervalThink(self:GetAbility().legendary_waste_interval)
end
...
CustomGameEventManager:Send_ServerToPlayer(PlayerResource:GetPlayer(self:GetParent():GetPlayerOwnerID()), 'leshrac_storm_change',  {max = self:GetAbility().legendary_max, damage = self:GetStackCount()})
end
...
if self:GetParent() == params.unit and not params.attacker:IsBuilding() and (not params.attacker:IsMagicImmune())
 ...
 and (self:GetParent():GetAbsOrigin() - params.attacker:GetAbsOrigin()):Length2D() <= self:GetAbility().knockback_radius
 and not params.attacker:HasModifier("modifier_leshrac_lightning_storm_custom_knockback_cd") then 


  params.attacker:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_leshrac_lightning_storm_custom_knockback_cd", {duration = self:GetAbility().knockback_cd})
  ...
  params.attacker:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_leshrac_lightning_storm_custom_knockback", {duration = self:GetAbility().knockback_duration})
...
if self:GetParent():HasModifier("modifier_leshrac_storm_4") and params.inflictor == nil and (not params.unit:IsMagicImmune() or self:GetCaster():HasModifier("modifier_leshrac_storm_6"))
  and RollPseudoRandomPercentage(self:GetAbility().attack_chance[self:GetParent():GetUpgradeStack("modifier_leshrac_storm_4")],247,self:GetParent()) then 

  self:GetCaster():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_leshrac_lightning_storm_custom", {damage = self:GetAbility().attack_damage, slow = self:GetAbility().attack_slow, starting_unit_entindex = params.unit:entindex(), bounces = self:GetAbility().attack_bounce})
end
...
function modifier_leshrac_lightning_storm_custom_knockback:OnCreated(params)
  ...
  local point = self:GetCaster():GetAbsOrigin() + dir*self:GetAbility().knockback_distance_max

  self.knockback_distance = math.max(self:GetAbility().knockback_distance_min, (point - self:GetParent():GetAbsOrigin()):Length2D())
