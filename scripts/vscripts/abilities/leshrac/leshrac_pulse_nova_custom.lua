...
leshrac_pulse_nova_custom.damage_inc = {0.02, 0.03, 0.04}
...
leshrac_pulse_nova_custom.mana_cost = {-0.1, -0.15, -0.2}
...
leshrac_pulse_nova_custom.spells_mana = 0.10
...
if self:GetCaster():HasModifier("modifier_leshrac_nova_3") then 
  mana_cost = mana_cost*(1 + self:GetAbility().mana_cost[self:GetCaster():GetUpgradeStack("modifier_leshrac_nova_3")])
end
...
return self:GetSpecialValueFor("channel_time") + FrameTime()*2
end
...
self.total_mana = self:GetAbility():GetSpecialValueFor("total_mana")*self:GetParent():GetMaxMana()/100
self.total_time = self:GetAbility():GetSpecialValueFor("channel_time")
...
local mana = math.min(self.mana_tick, self:GetParent():GetMaxMana() - self:GetParent():GetMana())
...
print(self:GetStackCount(), damage , heal)
