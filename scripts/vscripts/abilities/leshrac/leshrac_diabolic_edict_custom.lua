...
leshrac_diabolic_edict_custom.heal_amount = 0.4
leshrac_diabolic_edict_custom.heal_amount_creeps = 0.33
leshrac_diabolic_edict_custom.heal_health = 30

leshrac_diabolic_edict_custom.legendary_radius = 500
leshrac_diabolic_edict_custom.legendary_duration = 3
leshrac_diabolic_edict_custom.legendary_stun = 1.5
leshrac_diabolic_edict_custom.legendary_damage = 2
leshrac_diabolic_edict_custom.legendary_damage_duration = 6
...
if self:GetCaster():HasModifier("modifier_leshrac_edict_5") then 
...
if self:GetCaster():HasModifier("modifier_leshrac_edict_7") then 
  local enemies = FindUnitsInRadius(self:GetCaster():GetTeamNumber(), self:GetCaster():GetOrigin(), nil, self.legendary_radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, 0, 0, false )

  for _,enemy in pairs(enemies) do 
    enemy:AddNewModifier(self:GetCaster(), self, "modifier_leshrac_diabolic_edict_custom_legendary", {duration = self.legendary_duration})
  end
...
if unit:HasModifier("modifier_leshrac_diabolic_edict_custom_legendary_damage") then 
  ...
  damage = damage + mod:GetStackCount()*self.legendary_damage
...
local heal = params.damage*self:GetAbility().heal_amount
if params.unit:IsCreep() then 
  heal = heal*self:GetAbility().heal_amount_creeps
end
...
if (self:GetCaster():GetAbsOrigin() - self:GetParent():GetAbsOrigin()):Length2D() > self:GetAbility().legendary_radius
...
self:GetParent():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_leshrac_diabolic_edict_custom_legendary_damage", {duration = self:GetAbility().legendary_damage_duration})
self:GetParent():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_stunned", {duration = self:GetAbility().legendary_stun*(1 - self:GetParent():GetStatusResistance())})
end
...
return self:GetAbility().legendary_damage*self:GetStackCount()
end
