...
lina_light_strike_array_custom.shard_damage = 15
...
function modifier_lina_light_strike_array_custom:OnDestroy()
  ...
  for _,enemy in pairs(enemies) do
    ...
    if enemy:IsRealHero() and self:GetCaster():GetQuest() == "Lina.Quest_6" then 
    ...
    if self:GetCaster():GetQuest() == "Lina.Quest_7" and enemy:IsRealHero() and not self:GetCaster():QuestCompleted() then 
      enemy:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_lina_fiery_soul_custom_quest", {duration = self:GetCaster().quest.number})
    end
