...
lina_laguna_blade_custom.shard_damage = 15
...
lina_laguna_blade_custom.magic_inc = {6, 9, 12}
...
lina_laguna_blade_custom.legendary_interval = 0.4
lina_laguna_blade_custom.legendary_delay = 0.4
lina_laguna_blade_custom.legendary_aoe = 125
lina_laguna_blade_custom.legendary_damage = 0.20
lina_laguna_blade_custom.legendary_damage_count = 320

lina_laguna_blade_custom.damage_heal = {0.30,0.45,0.60}
lina_laguna_blade_custom.damage_heal_creeps = 0.25

lina_laguna_blade_custom.shield_damage = -20
...
lina_laguna_blade_custom.damage_steal = {0.02, 0.03}
lina_laguna_blade_custom.damage_steal_duration = 10
lina_laguna_blade_custom.damage_steal_creeps = 0.25
...
self.RemoveForDuel = true 
self.damage = self:GetAbility().magic_inc[self:GetCaster():GetUpgradeStack("modifier_lina_laguna_2")]
end
...
if self:GetParent() == params.attacker  and self:GetParent():HasModifier("modifier_lina_laguna_legendary") and not params.unit:IsIllusion()
...
while damage > 0 do 
  ...
  if self:GetStackCount() < self:GetAbility().damage_damage then 
    ...
    self:GetParent():AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_lina_laguna_blade_custom_spell_steal_count", {duration = self:GetAbility().damage_steal_duration})
    self:GetParent():AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_lina_laguna_blade_custom_spell_steal", {duration = self:GetAbility().damage_steal_duration})
...
function modifier_lina_laguna_blade_custom_spell_steal:OnTakeDamage(params)
if not IsServer() then return end
...
local heal = self:GetAbility().damage_steal[self:GetCaster():GetUpgradeStack("modifier_lina_laguna_4")]*self:GetStackCount()*params.damage

if params.unit:IsCreep() then 
  heal = heal*self:GetAbility().damage_steal_creeps
end
...
function modifier_lina_laguna_blade_custom_spell_steal:OnTooltip()
return self:GetAbility().damage_steal[self:GetCaster():GetUpgradeStack("modifier_lina_laguna_4")]*self:GetStackCount()*100
end
