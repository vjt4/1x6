...
marci_grapple_custom.str_damage = {0.4, 0.6, 0.8}

marci_grapple_custom.combo_duration = 3
marci_grapple_custom.combo_status = 20
marci_grapple_custom.combo_move = 20

marci_grapple_custom.slow_move = {-10, -15, -20}
marci_grapple_custom.slow_duration = {1, 1.5, 2}

marci_grapple_custom.attack_chance = {10, 15, 20}
marci_grapple_custom.attack_damage = 50
marci_grapple_custom.attack_slow = 1 

marci_grapple_custom.str_change = 0.12
marci_grapple_custom.str_duration = 10

marci_grapple_custom.heal_chance = {25, 40}
...
if self:GetCaster():HasModifier("modifier_marci_dispose_1") then 
	damage = damage + self:GetCaster():GetStrength()*self.str_damage[self:GetCaster():GetUpgradeStack("modifier_marci_dispose_1")]
end
...
function marci_grapple_custom:OnSpellStart(new_target)
	...
	if self:GetCaster():HasModifier("modifier_marci_dispose_5") then 
		...
		local str = self:GetCaster():GetStrength()*self.str_change

		if original_target:IsHero() then 
			str = original_target:GetStrength()*self.str_change
			local mod = original_target:AddNewModifier(self:GetCaster(), self, "modifier_marci_dispose_str", {duration = self.str_duration})
			
			mod:SetStackCount(str)
			original_target:CalculateStatBonus(true)
		end

		local mod = self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_marci_dispose_str", {duration = self.str_duration})
	...
	for _,target in pairs(targets) do 
		...
		arc:SetEndCallback( function()
			...
			if caster:HasShard() then 
				target:AddNewModifier(caster, self, "modifier_marci_dispose_disarm", {duration = (1 - target:GetStatusResistance())*self:GetSpecialValueFor("shard_disarm")})
			end
...
local trees = GridNav:GetAllTreesAroundPoint( self:GetParent():GetAbsOrigin(), self.parent:GetHullRadius(), true )
...
if #trees > 0 or self:GetParent():IsCreep() then 
	self.tree_stun = true
...
local chance = self:GetAbility().attack_chance[self:GetCaster():GetUpgradeStack("modifier_marci_dispose_3")]


local random = RollPseudoRandomPercentage(chance,101,self:GetParent())
if not random then return end

local damageTable = {victim = params.target, attacker = self:GetParent(), damage = self:GetAbility():GetDisposeDamage(self:GetAbility().attack_damage), damage_type = DAMAGE_TYPE_MAGICAL, ability = self:GetAbility() }
ApplyDamage(damageTable)
params.target:AddNewModifier( self:GetParent(), self:GetAbility(), "modifier_marci_dispose_slow", { duration = self:GetAbility().attack_slow*(1 - params.target:GetStatusResistance()) } )
...
if self:GetParent():HasModifier("modifier_marci_dispose_6") then
	self:GetParent():AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_marci_dispose_cast_attack", {duration = self:GetAbility().combo_duration})
end
