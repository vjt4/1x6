...
LinkLuaModifier( "modifier_marci_companion_run_custom_debuff", "abilities/marci/marci_companion_run_custom", LUA_MODIFIER_MOTION_NONE )
...
marci_companion_run_custom.legendary_duration = 5
...
marci_companion_run_custom.root_debuff_duration = 6
marci_companion_run_custom.root_stun = 0.5
marci_companion_run_custom.root_duration = 2
marci_companion_run_custom.root_hits = 6

marci_companion_run_custom.range_attacks_duration = {3,5}
marci_companion_run_custom.range_attacks_bonus = 200
marci_companion_run_custom.range_attacks_speed = 200
...
if self:GetCaster():HasModifier("modifier_marci_rebound_5") then 
	debuff = self.root_debuff_duration
	stun = stun + self.root_stun
end

if self:GetCaster():HasModifier("modifier_marci_rebound_4") then 
	self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_marci_companion_run_custom_attacks", {duration = self.range_attacks_duration[self:GetCaster():GetUpgradeStack("modifier_marci_rebound_4")]})
end
...
for _,enemy in pairs(enemies) do
	...
	enemy:AddNewModifier( self:GetCaster(), self, "modifier_marci_companion_run_custom_debuff", { duration = debuff } )

	if enemy:IsRealHero() and self:GetCaster():GetQuest() == "Marci.Quest_6" then 
		self:GetCaster():UpdateQuest(1)
	end

	enemy:AddNewModifier(self:GetCaster(), self, "modifier_stunned", {duration = (1 - enemy:GetStatusResistance())*self:GetSpecialValueFor("stun_duration")})
end
...
modifier_marci_companion_run_custom_debuff = class({})

function modifier_marci_companion_run_custom_debuff:IsHidden()
	return true
end

function modifier_marci_companion_run_custom_debuff:IsDebuff()
	return true
end

function modifier_marci_companion_run_custom_debuff:IsPurgable()
	return true
end

function modifier_marci_companion_run_custom_debuff:OnCreated( kv )
if not IsServer() then return end
	self:GetParent():EmitSound("Hero_Marci.Rebound.Ally")
end

function modifier_marci_companion_run_custom_debuff:OnRefresh( kv )
	self:OnCreated( kv )
end

function modifier_marci_companion_run_custom_debuff:DeclareFunctions()
	local funcs = {
...
function modifier_marci_companion_run_custom_debuff:OnAttackLanded(params)
if not IsServer() then return end
if not self:GetCaster():HasModifier("modifier_marci_rebound_5") then return end
if self:GetParent() ~= params.target then return end
if self:GetCaster() ~= params.attacker then return end
if self:GetParent():HasModifier("modifier_marci_companion_run_custom_root") then return end
if self:GetCaster():HasModifier("modifier_marci_companion_run_custom_no_root") then return end

self:IncrementStackCount()
if self:GetStackCount() >= self:GetAbility().root_hits then 
	self:SetStackCount(0)
	self:GetParent():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_marci_companion_run_custom_root", {duration = (1 - self:GetParent():GetStatusResistance())*self:GetAbility().root_duration})
	self:Destroy()
end
...
function modifier_marci_companion_run_custom_debuff:GetEffectName()
	return "particles/units/heroes/hero_marci/marci_rebound_bounce_impact_debuff.vpcf"
end

function modifier_marci_companion_run_custom_debuff:GetEffectAttachType()
	return PATTACH_ABSORIGIN_FOLLOW
end

function modifier_marci_companion_run_custom_debuff:GetStatusEffectName()
	return "particles/status_fx/status_effect_snapfire_slow.vpcf"
end

function modifier_marci_companion_run_custom_debuff:StatusEffectPriority()
...
if not mod.target then return end
if mod.target:IsNull() then return end

self.parent = self:GetCaster()

self.direction = mod.target:GetAbsOrigin() - self:GetCaster():GetAbsOrigin() 
self.direction.z = 0
self.direction = self.direction:Normalized()


self.target_abs = mod.target:GetAbsOrigin() + self.direction * 120
...
arc:SetEndCallback( function( interrupted )
	...
	local damageTable = { attacker = self.parent, damage = self.damage, damage_type = DAMAGE_TYPE_MAGICAL, ability = self.ability, }
...
function modifier_marci_companion_run_custom_legendary_second:IsHidden() return false end
...
--local effect_cast = ParticleManager:CreateParticle( "particles/lc_odd_charge_mark.vpcf", PATTACH_OVERHEAD_FOLLOW, self.target )
...
self:SetStackCount(self:GetStackCount() + params.damage*self:GetAbility().legendary_damage)
...
function modifier_marci_companion_run_custom_attacks:GetModifierAttackRangeBonus()
return self:GetAbility().range_attacks_bonus
end


function modifier_marci_companion_run_custom_attacks:GetModifierAttackSpeedBonus_Constant()
return self:GetAbility().range_attacks_speed
end

function modifier_marci_companion_run_custom_attacks:OnAttackLanded(params)
if not IsServer() then return end
if self:GetParent() ~= params.attacker then return end
