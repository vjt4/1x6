...
marci_unleash_custom.heal_silence = 1

marci_unleash_custom.combo_duration = 4
marci_unleash_custom.combo_max = 8
marci_unleash_custom.combo_damage = {50, 75}
...
for _,enemy in pairs(enemies) do
	...
	if enemy:HasModifier("modifier_marci_unleash_custom_combo") then 
		final_damage = final_damage + enemy:FindModifierByName("modifier_marci_unleash_custom_combo"):GetStackCount()*self.combo_damage[self:GetCaster():GetUpgradeStack("modifier_marci_unleash_4")]
...
if self:GetParent():HasModifier("modifier_marci_companion_run_custom_no_root") then return end
	...
	if self.counter<=0 then
		...
		if self:GetCaster():HasModifier("modifier_marci_unleash_4") then 
			params.target:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_marci_unleash_custom_combo", {duration = self:GetAbility().combo_duration})
		...
		if self:GetCaster():HasModifier("modifier_marci_unleash_5") then 
			...
			params.target:AddNewModifier( self:GetCaster(), self:GetAbility(), "modifier_marci_unleash_custom_silence", { duration = self:GetAbility().heal_silence*(1 - params.target:GetStatusResistance()) } )
		end
...
if not IsServer() then return end
if self:GetStackCount() >= self:GetAbility().combo_max then return end
...
return self:GetStackCount()*self:GetAbility().combo_damage[self:GetCaster():GetUpgradeStack("modifier_marci_unleash_4")]
end
