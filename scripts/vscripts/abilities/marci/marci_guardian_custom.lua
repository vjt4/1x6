...
marci_guardian_custom.lifesteal_inc = {0.1, 0.2, 0.3}
marci_guardian_custom.lifesteal_heal = {0.08, 0.12, 0.16}

marci_guardian_custom.death_heal = 0.25

marci_guardian_custom.dispel_speed = 550
marci_guardian_custom.dispel_duration = 1.8

marci_guardian_custom.armor_inc = {0.2, 0.3, 0.4}
...
function marci_guardian_custom:OnSpellStart()
	...
	caster:AddNewModifier( caster, self, "modifier_marci_guardian_custom", { duration = duration } )
...
function modifier_marci_guardian_custom:OnCreated( kv )
	...
	if self:GetCaster():HasModifier("modifier_marci_sidekick_3") then 
		self:GetParent():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_marci_guardian_custom_armor", {duration = self:GetRemainingTime()})
	end


	if self.caster:HasModifier("modifier_marci_sidekick_2") then 
		self.lifesteal = self.lifesteal + self:GetAbility().lifesteal_inc[self:GetCaster():GetUpgradeStack("modifier_marci_sidekick_2")]

		local heal = self:GetParent():GetMaxHealth()*self:GetAbility().lifesteal_heal[self:GetCaster():GetUpgradeStack("modifier_marci_sidekick_2")]
...
if not IsServer() then return end
	...
	if self.caster:HasModifier("modifier_marci_sidekick_6")then 
		self.parent:AddNewModifier(self.caster, self:GetAbility(), "modifier_marci_guardian_custom_speed", {duration = self:GetAbility().dispel_duration})
	end
...
function modifier_marci_guardian_custom:PlayEffects2()
...
function modifier_marci_guardian_custom_speed:GetModifierMoveSpeed_Absolute()
	return self:GetAbility().dispel_speed
end
...
modifier_marci_guardian_custom_armor = class({})
function modifier_marci_guardian_custom_armor:IsHidden() return true end
function modifier_marci_guardian_custom_armor:IsPurgable() return false end
function modifier_marci_guardian_custom_armor:OnCreated(table)
if not IsServer() then return end
self:SetStackCount(self:GetCaster():GetPhysicalArmorValue(false)*self:GetAbility().armor_inc[self:GetCaster():GetUpgradeStack("modifier_marci_sidekick_3")])
end
