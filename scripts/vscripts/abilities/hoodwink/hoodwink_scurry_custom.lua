...
hoodwink_scurry_custom.legendary_duration = 3
hoodwink_scurry_custom.legendary_max = 6
hoodwink_scurry_custom.legendary_distance = 133
hoodwink_scurry_custom.legendary_agility = 30
...
hoodwink_scurry_custom.attacks_chance_inc = 15
hoodwink_scurry_custom.attacks_damage = 0.6

hoodwink_scurry_custom.vision_timer = 3
hoodwink_scurry_custom.vision_range = 150
hoodwink_scurry_custom.vision_duration = 3
...
if self:GetParent() ~= params.attacker then return end
	...
	if not params.target:IsBuilding() and not params.target:IsMagicImmune() and self:GetParent():HasModifier("modifier_hoodwink_scurry_4") and not self:GetParent():HasModifier("modifier_hoodwink_acorn_shot_custom") then 
		...
		if random and params.target:GetUnitName() ~= "npc_teleport" then 
...
function modifier_hoodwink_scurry_custom:OnIntervalThink()
	...
	if #trees > 0 and self:GetParent():HasModifier("modifier_hoodwink_scurry_custom_vision") then
		self:GetParent():AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_hoodwink_scurry_custom_vision", {duration = self:GetAbility().vision_duration})
	end
...
self.agi_percentage = self:GetAbility().legendary_agility
self:OnIntervalThink()
...
function modifier_hoodwink_scurry_custom_legendary:GetModifierBonusStats_Agility()
return self.agility
end

function modifier_hoodwink_scurry_custom_legendary:OnIntervalThink()
if not IsServer() then return end
...
return self:GetAbility().legendary_agility
end
...
CustomGameEventManager:Send_ServerToPlayer(PlayerResource:GetPlayer(self:GetParent():GetPlayerOwnerID()), 'Hoodwink_change',  {max = self:GetAbility().legendary_max * self:GetAbility().legendary_distance, current = 0})
...
local max = self:GetAbility().legendary_max * self:GetAbility().legendary_distance
...
function modifier_hoodwink_scurry_custom_vision:GetModifierAttackRangeBonus()
return self:GetAbility().vision_range
end
