...
hoodwink_bushwhack_custom.cd_init = 0
hoodwink_bushwhack_custom.cd_inc = -2

hoodwink_bushwhack_custom.damage_inc = {80, 120, 160}
...
hoodwink_bushwhack_custom.incoming_inc = {10, 15}
hoodwink_bushwhack_custom.incoming_duration = 15
hoodwink_bushwhack_custom.incoming_max = 8
...
hoodwink_bushwhack_custom.silence_duration = 1.5
hoodwink_bushwhack_custom.silence_mana = 20
...
function modifier_hoodwink_bushwhack_custom_debuff:OnCreated( kv )
	...
	if self:GetCaster():HasModifier("modifier_hoodwink_bush_2") then 
		self.damage = self.damage + self:GetAbility().damage_inc[self:GetCaster():GetUpgradeStack("modifier_hoodwink_bush_2")]
	end
...
function modifier_hoodwink_bushwhack_custom_debuff:OnIntervalThink()
	...
	if self:GetCaster():HasModifier("modifier_hoodwink_bush_5") then 
		local mana = (self:GetAbility().silence_mana*self:GetParent():GetMaxMana()/100)/(self.tick_count)
...
return self:GetStackCount()*(self:GetAbility().incoming_inc[self:GetCaster():GetUpgradeStack("modifier_hoodwink_bush_4")])
end
