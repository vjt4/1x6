...
bloodseeker_bloodrage_custom.speed_move = 25
bloodseeker_bloodrage_custom.speed_cdr = 25
bloodseeker_bloodrage_custom.speed_lowhp = 20
...
ApplyDamage({attacker = self:GetCaster(), victim = self:GetParent(), damage_type = DAMAGE_TYPE_PURE, ability = self:GetAbility(), damage = self_damage, damage_flags = DOTA_DAMAGE_FLAG_HPLOSS + DOTA_DAMAGE_FLAG_NO_DAMAGE_MULTIPLIERS + DOTA_DAMAGE_FLAG_NON_LETHAL})
end
...
function modifier_bloodseeker_bloodrage_custom:GetModifierProcAttack_BonusDamage_Pure( params )
if not IsServer() then return end
...
function modifier_bloodseeker_bloodrage_tracker:GetModifierMoveSpeedBonus_Percentage()
local bonus = 0
if self:GetCaster():HasModifier("modifier_bloodseeker_bloodrage_5") then 
	local k = math.min(1, (100 - self:GetParent():GetHealthPercent())/(100 - self:GetAbility().speed_lowhp))
	bonus = self:GetAbility().speed_move*k
end

 	return bonus
end

function modifier_bloodseeker_bloodrage_tracker:GetModifierPercentageCooldown() 
local bonus = 0
if self:GetCaster():HasModifier("modifier_bloodseeker_bloodrage_5") then 
	local k = math.min(1, (100 - self:GetParent():GetHealthPercent())/(100 - self:GetAbility().speed_lowhp))
	bonus = self:GetAbility().speed_cdr*k
end
...
if self:GetAbility() == params.inflictor and params.unit ~= self:GetParent() then 
