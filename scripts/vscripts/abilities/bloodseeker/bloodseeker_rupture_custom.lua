...
LinkLuaModifier("modifier_bloodseeker_rupture_custom_blood_heal", "abilities/bloodseeker/bloodseeker_rupture_custom", LUA_MODIFIER_MOTION_NONE)
...
bloodseeker_rupture_custom.blood_damage = {0.02, 0.03}
bloodseeker_rupture_custom.blood_damage_creeps = 0.33
bloodseeker_rupture_custom.blood_damage_max = 500
bloodseeker_rupture_custom.blood_damage_interval = 0.5
bloodseeker_rupture_custom.blood_heal = 1
...
self.target:AddNewModifier(self:GetCaster(), self, "modifier_bloodseeker_rupture_custom_legendary", {duration = self:GetSpecialValueFor("duration")})
end
...
self.heal = self:GetParent():GetMaxHealth()*self:GetAbility():GetSpecialValueFor("heal")/(100 * self.knockback_count)
...
if not IsServer() then return end
my_game:GenericHeal(self:GetCaster(), self.heal, self:GetAbility())
...
self.damage = self:GetParent():GetMaxHealth()*self:GetAbility().blood_damage[self:GetCaster():GetUpgradeStack("modifier_bloodseeker_rupture_4")]*self:GetAbility().blood_damage_interval
if self:GetParent():IsCreep() then 

	self.damage = self.damage*self:GetAbility().blood_damage_creeps
	self.damage = math.min(self:GetAbility().blood_damage_max*self:GetAbility().blood_damage_interval, self.damage)
end
...
self:GetCaster():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_bloodseeker_rupture_custom_blood_heal", {})
...
self:GetCaster():RemoveModifierByName("modifier_bloodseeker_rupture_custom_blood_heal")
end


modifier_bloodseeker_rupture_custom_blood_heal = class({})
function modifier_bloodseeker_rupture_custom_blood_heal:IsHidden() return true end
function modifier_bloodseeker_rupture_custom_blood_heal:IsPurgable() return false end
function modifier_bloodseeker_rupture_custom_blood_heal:DeclareFunctions()
return
{
	MODIFIER_EVENT_ON_TAKEDAMAGE
}
...
function modifier_bloodseeker_rupture_custom_blood_heal:OnTakeDamage(params)
if not IsServer() then return end
if self:GetParent() ~= params.attacker then return end
if params.inflictor ~= self:GetAbility() then return end
if params.unit:IsIllusion() then return end

my_game:GenericHeal(self:GetCaster(), params.damage*self:GetAbility().blood_heal, self:GetAbility())
end
