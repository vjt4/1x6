...
bloodseeker_blood_bath_custom.dispel_bkb = 1.2

bloodseeker_blood_bath_custom.blood_damage = {0.15, 0.2, 0.25}
bloodseeker_blood_bath_custom.blood_duration = 4
bloodseeker_blood_bath_custom.blood_interval = 1
...
bloodseeker_blood_bath_custom.legendary_max = 6
bloodseeker_blood_bath_custom.legendary_interval = 0.25
bloodseeker_blood_bath_custom.legendary_damage = 0.9
bloodseeker_blood_bath_custom.legendary_self = 0.03

bloodseeker_blood_bath_custom.attack_root = {0.8, 1.2}
bloodseeker_blood_bath_custom.attack_duration = 5
bloodseeker_blood_bath_custom.attack_str_duration = 6
bloodseeker_blood_bath_custom.attack_str = {0.08, 0.12}
...
if self:GetCaster():HasModifier("modifier_bloodseeker_bloodrite_7") and ability_end:IsHidden() then 
	...
	self:GetCaster():FindAbilityByName("bloodseeker_blood_bath_custom_end"):StartCooldown(0.3)
end
...
if self:GetCaster():HasModifier("modifier_bloodseeker_bloodrite_7") and ability:IsHidden() then 
	self:GetCaster():SwapAbilities(self:GetName(), "bloodseeker_blood_bath_custom", false, true)
end
...
for _,enemy in pairs(enemies) do
	if enemy:GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then 
		...
		if self:GetCaster():HasModifier("modifier_bloodseeker_bloodrite_1") then
			enemy:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_bloodseeker_blood_bath_custom_slow_blood", {duration = self:GetAbility().blood_duration, damage = self.damage*self:GetAbility().blood_damage[self:GetCaster():GetUpgradeStack("modifier_bloodseeker_bloodrite_1")]})
		end
		...
		if self:GetCaster() == enemy then 
			if self:GetCaster():HasModifier("modifier_bloodseeker_bloodrite_5") then 
				...
				self:GetCaster():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_bloodseeker_blood_bath_custom_bkb", {duration = self:GetAbility().dispel_bkb})
			end
...
if self:GetCaster():HasModifier("modifier_bloodseeker_bloodrite_7") and self:GetAbility():IsHidden() then 
...
function modifier_bloodseeker_blood_bath_custom_slow_blood:IsHidden() return true end
function modifier_bloodseeker_blood_bath_custom_slow_blood:IsPurgable() return true end
function modifier_bloodseeker_blood_bath_custom_slow_blood:GetAttributes() return MODIFIER_ATTRIBUTE_MULTIPLE end
function modifier_bloodseeker_blood_bath_custom_slow_blood:GetEffectName() return "particles/items2_fx/sange_maim.vpcf" end

function modifier_bloodseeker_blood_bath_custom_slow_blood:OnCreated(table)
if not IsServer() then return end
...
function modifier_bloodseeker_blood_bath_custom_slow_blood:OnIntervalThink()
if not IsServer() then return end
