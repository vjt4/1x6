...
function modifier_item_patrol_fortifier_tower:OnIntervalThink()
if not IsServer() then return end

local enemies = FindUnitsInRadius(self:GetParent():GetTeamNumber(), self:GetParent():GetAbsOrigin(), nil, 780, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_CREEP, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + DOTA_UNIT_TARGET_FLAG_OUT_OF_WORLD + DOTA_UNIT_TARGET_FLAG_INVULNERABLE, FIND_CLOSEST, false)

for _,enemy in pairs(enemies) do 
 	if (enemy:IsRealHero() and not enemy:HasModifier("modifier_monkey_king_wukongs_command_custom_soldier")) or enemy:GetTeamNumber() == DOTA_TEAM_CUSTOM_5 then 
