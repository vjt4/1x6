...
if self:GetCaster():HasModifier("modifier_custom_pudge_dismember_no_tp") or self:GetCaster():HasModifier("modifier_snapfire_mortimer_kisses_custom_gobble_caster")
	or self:GetCaster():HasModifier("modifier_custom_puck_phase_shift") then 
 	CustomGameEventManager:Send_ServerToPlayer(PlayerResource:GetPlayer(self:GetCaster():GetPlayerOwnerID()), "CreateIngameErrorMessage", {message = "#cant_tp"})
	return false
end
