...
if #enemies > 0 then 
	...
	if self.state == 0 then 
		self.state = 1
   		self:GetParent():AddNewModifier(self:GetCaster(), nil, "modifier_tower_backdoor_timer", {duration = 2})
   	end 
...
function modifier_backdoor:GetAbsoluteNoDamagePhysical(params)
if not IsServer() then return end
local tower = towers[self:GetParent():GetTeamNumber()]
local attacker = params.attacker

if not tower or not attacker then return end

if (tower:GetAbsOrigin() - attacker:GetAbsOrigin()):Length2D() > 900  or not attacker:IsAlive() then 
	return 1
end
...
if players[attacker:GetTeamNumber()] then
	return 1
end
...
function modifier_backdoor:GetAbsoluteNoDamageMagical(params)
if not IsServer() then return end
local tower = towers[self:GetParent():GetTeamNumber()]
local attacker = params.attacker
...
function modifier_backdoor:GetAbsoluteNoDamagePure(params)
if not IsServer() then return end
local tower = towers[self:GetParent():GetTeamNumber()]
local attacker = params.attacker
...
if attacker:HasModifier("modifier_tower_incoming_damage") then return 0 end
if players[attacker:GetTeamNumber()] then
	return 1
end
...
function modifier_backdoor_knock_aura:GetAuraSearchTeam() return DOTA_UNIT_TARGET_TEAM_ENEMY  end
...
if params.attacker:GetTeamNumber() ~= self:GetCaster():GetTeamNumber() and players[params.attacker:GetTeamNumber()] then 
	return 1
end
