...
zuus_lightning_bolt_custom.resist_max = 3
zuus_lightning_bolt_custom.resist_duration = 8

zuus_lightning_bolt_custom.cd_reduction = {-0.5, -1, -1.5}
...
zuus_lightning_bolt_custom.near_heal = 0.4
...
zuus_lightning_bolt_custom.legendary_max = 10
zuus_lightning_bolt_custom.legendary_min = 3
...
function zuus_lightning_bolt_custom:CastLightningBolt(caster, target, target_point, not_cast, nimbus, hand, delayed)
	...
	if target == nil then
		...
		local nearby_enemy_units = FindUnitsInRadius( caster:GetTeamNumber(),  target_point,  nil,  spread_aoe,  DOTA_UNIT_TARGET_TEAM_ENEMY,  DOTA_UNIT_TARGET_HERO,  target_flags,  FIND_CLOSEST,  false )
	...
	if self:GetCaster():HasModifier("modifier_zuus_jump_4") and not_cast == false and target then 
	...
	if target ~= nil and target:GetTeam() ~= caster:GetTeam() then
		...
		if zuus_static_field_custom and zuus_static_field_custom:GetLevel() > 0 and (not_cast == false or nimbus) then
