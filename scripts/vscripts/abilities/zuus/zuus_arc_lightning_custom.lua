...
zuus_arc_lightning_custom.damage_inc = {20, 40, 60}

zuus_arc_lightning_custom.legendary_max_target = 2
zuus_arc_lightning_custom.legendary_max_heal = 0.4
zuus_arc_lightning_custom.legendary_damage = 0.4

zuus_arc_lightning_custom.attack_chance = {10, 15, 20}
...
zuus_arc_lightning_custom.stack_max = 4
zuus_arc_lightning_custom.stack_stun = {0.8, 1.2}
zuus_arc_lightning_custom.stack_damage = {0.025, 0.04}
zuus_arc_lightning_custom.stack_duration = 10
...
zuus_arc_lightning_custom.cd_damage = -15
...
function zuus_arc_lightning_custom:OnSpellStart(new_target)
	...
	if self:GetCaster():HasModifier("modifier_zuus_arc_4") then 
		self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_zuus_arc_lightning_attack_stack", {duration = self.stack_duration})
	end
...
function zuus_arc_lightning_custom:StartArc(target, bounces, not_cast)
if not IsServer() then return end
...
self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_zuus_arc_lightning_custom", {not_cast = not_cast, starting_unit_entindex = target:entindex(), bounces = bounces })
end
...
function modifier_zuus_arc_lightning_custom:OnCreated(keys)
	...
	if self:GetCaster():HasModifier("modifier_zuus_arc_1") then 
		self.arc_damage = self.arc_damage + self:GetAbility().damage_inc[self:GetCaster():GetUpgradeStack("modifier_zuus_arc_1")]
	end
	...
	if self:GetCaster():HasModifier("modifier_zuus_arc_7") then 
		self.max_per_target = self:GetAbility().legendary_max_target
	end
	...
	if self.starting_unit_entindex and EntIndexToHScript(self.starting_unit_entindex) then
		...
		self:DoDamage(self.current_unit, self.not_cast)
...
if zuus_static_field_custom and zuus_static_field_custom:GetLevel() > 0 and (not_cast == 0 or not_cast == false) then
	zuus_static_field_custom:ApplyDamageStatic(target)
end
	...
	for _, enemy in pairs(FindUnitsInRadius(self:GetCaster():GetTeamNumber(), self.current_unit:GetAbsOrigin(), nil, self.radius, team, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE + DOTA_UNIT_TARGET_FLAG_NO_INVIS, FIND_CLOSEST, false)) do
		...
		if (not self.units_affected[enemy] or self.units_affected[enemy] < self.max_per_target) 
			...
			if enemy:GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then 
				...
				local heal = self:GetAbility().legendary_max_heal*(1 - enemy:GetHealth()/enemy:GetMaxHealth())
				my_game:GenericHeal(enemy, self.arc_damage*heal, self)
			end
...
local damage = target:GetMaxHealth()*(self:GetAbility().stack_damage[self:GetCaster():GetUpgradeStack("modifier_zuus_arc_4")])*self:GetStackCount()
...
if self:GetStackCount() == self:GetAbility().stack_max then 
	...
	target:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_stunned", {duration = (1 - target:GetStatusResistance())*self:GetAbility().stack_stun[self:GetCaster():GetUpgradeStack("modifier_zuus_arc_4")]})
...
return 100*(self:GetAbility().stack_damage[self:GetCaster():GetUpgradeStack("modifier_zuus_arc_4")])*self:GetStackCount()
end
...
CustomGameEventManager:Send_ServerToPlayer(PlayerResource:GetPlayer(self:GetParent():GetPlayerOwnerID()), 'zuus_bolt_change',  {max = self:GetAbility().stack_max, damage = stack})
end