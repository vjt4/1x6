...
zuus_heavenly_jump_custom.legendary_range = 100
...
zuus_heavenly_jump_custom.speed_attack = {15, 25}
...
target:AddNewModifier(self:GetCaster(), self, "modifier_zuus_heavenly_jump_custom_debuff", {duration = duration})
...
function modifier_zuus_heavenly_jump_custom_debuff:OnCreated()
	...
	self.aspd_slow = self:GetAbility():GetSpecialValueFor("aspd_slow") * -1
	self.cast_slow = self:GetAbility():GetSpecialValueFor("cast_slow") * -1
end

function modifier_zuus_heavenly_jump_custom_debuff:OnRefresh()
	self.move_slow = self:GetAbility():GetSpecialValueFor("move_slow") * -1
	self.aspd_slow = self:GetAbility():GetSpecialValueFor("aspd_slow") * -1
	self.cast_slow = self:GetAbility():GetSpecialValueFor("cast_slow") * -1
end
...
function modifier_zuus_heavenly_jump_custom_debuff:GetModifierPercentageCasttime()
...
function modifier_zuus_heavenly_jump_custom_move_passive:GetModifierAttackRangeBonus()
if not self:GetParent():HasModifier("modifier_zuus_jump_7") then return end
return self:GetAbility().legendary_range
end
...
for _,unit in pairs(units) do 
	...
	unit:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_generic_silence", {duration = (1 - unit:GetStatusResistance())*self:GetAbility().illusion_silence})
end
