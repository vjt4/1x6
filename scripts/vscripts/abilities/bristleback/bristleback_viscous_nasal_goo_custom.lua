...
bristleback_viscous_nasal_goo_custom.legendary_damage = 0.015
...
bristleback_viscous_nasal_goo_custom.damage_duration = {1, 2, 3}

bristleback_viscous_nasal_goo_custom.dispel_stun = 1
bristleback_viscous_nasal_goo_custom.dispel_max = 4

bristleback_viscous_nasal_goo_custom.stacks_stacks = 2
bristleback_viscous_nasal_goo_custom.stacks_chance = 15
...
function bristleback_viscous_nasal_goo_custom:OnSpellStart(target, unit)
  ...
  if self:GetCaster():HasModifier("modifier_bristle_goo_stack") 
    and RollPseudoRandomPercentage(self.stacks_chance ,119,self:GetCaster()) then 
...
if hTarget == nil or not hTarget:IsAlive() or hTarget:IsMagicImmune() then return end
    ...
    if self:GetCaster():HasModifier("modifier_bristle_goo_damage") then 
      ...
      local chance = self.ground_chance[self:GetCaster():GetUpgradeStack("modifier_bristle_goo_damage")]
...
return self:GetAbility().ground_armor
end
