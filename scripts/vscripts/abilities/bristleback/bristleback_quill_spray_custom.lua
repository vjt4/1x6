...
bristleback_quill_spray_custom.proc_slow = -40
...
bristleback_quill_spray_custom.lowhp_health = 30
...
if self:GetCaster():HasModifier("modifier_bristle_spray_heal") then 
  my_game:GenericHeal(self:GetCaster(), self.heal[self:GetCaster():GetUpgradeStack("modifier_bristle_spray_heal")]*self:GetCaster():GetMaxHealth(), self)
end
...
function bristleback_quill_spray_custom:MakeSpray(location, double, active)
  ...
  if self:GetCaster():HasModifier("modifier_bristle_spray_double") and (double == true or double == nil) and active  then 
