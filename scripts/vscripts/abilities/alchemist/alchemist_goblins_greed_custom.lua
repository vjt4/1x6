...
alchemist_goblins_greed_custom.bonus_speed = 2
alchemist_goblins_greed_custom.bonus_attribute_maxstacks = {20, 35}

alchemist_goblins_greed_custom.blue = {2,3,4}


alchemist_goblins_greed_custom.active_manacost = 100
alchemist_goblins_greed_custom.active_cooldown = {160, 120, 80}
...
function alchemist_goblins_greed_custom:GetBehavior()
	if self:GetCaster():HasModifier("modifier_alchemist_greed_3") then
...
function alchemist_goblins_greed_custom:GetCooldown(iLevel)
	if self:GetCaster():HasModifier("modifier_alchemist_greed_3") then
    	return self.active_cooldown[self:GetCaster():GetUpgradeStack("modifier_alchemist_greed_3")]
    end
    return 0
end
...
function modifier_alchemist_goblins_greed_custom_inventory_attribute:GetModifierMoveSpeedBonus_Constant()
	return self:GetStackCount() * self:GetAbility().bonus_speed
end
...
function modifier_alchemist_goblins_greed_custom_runes:IsHidden() return false end
