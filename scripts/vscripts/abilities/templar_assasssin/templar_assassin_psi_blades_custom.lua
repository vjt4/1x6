...
templar_assassin_psi_blades_custom.heal_distance = 300
templar_assassin_psi_blades_custom.heal_attack = {30, 45, 60}

templar_assassin_psi_blades_custom.legendary_hits = 4
templar_assassin_psi_blades_custom.legendary_duration = 8
templar_assassin_psi_blades_custom.legendary_stun = 1.2
templar_assassin_psi_blades_custom.legendary_range = 350
templar_assassin_psi_blades_custom.legendary_castrange = 1200
templar_assassin_psi_blades_custom.legendary_slow = -65
templar_assassin_psi_blades_custom.legendary_slow_duration = 2
templar_assassin_psi_blades_custom.legendary_cd = 12

templar_assassin_psi_blades_custom.crystal_stun = 0.8
templar_assassin_psi_blades_custom.crystal_damage = 0.10
templar_assassin_psi_blades_custom.crystal_chance = {10, 15}
templar_assassin_psi_blades_custom.crystal_duration = 5
...
crystal:AddNewModifier(self:GetCaster(), self, "modifier_templar_assassin_psi_blades_custom_legendary", {target = target:entindex()})
crystal:AddNewModifier(self:GetCaster(), self, "modifier_kill", {duration = self.legendary_duration})

crystal:SetBaseMaxHealth(self.legendary_hits)
crystal:SetHealth(self.legendary_hits)
...
if self.target and not self.target:IsNull() then 
	AddFOWViewer(self:GetCaster():GetTeamNumber(), self.target:GetAbsOrigin(), 10, 0.2, false)
end
...
function modifier_templar_assassin_psi_blades_custom_legendary:OnAttackLanded( param )
if not IsServer() then return end
...
if self.stun == true and self.target and not self.target:IsNull() and not self.target:IsInvulnerable() then 
 

	self:GetParent():EmitSound("TA.Psibaldes_crystall_end_stun")

	local explode_particle = ParticleManager:CreateParticle("particles/ta_crystal_end.vpcf", PATTACH_WORLDORIGIN, nil)
	ParticleManager:SetParticleControl(explode_particle, 0, self.target:GetAbsOrigin())
	ParticleManager:SetParticleControl(explode_particle, 60, Vector(12,198,255))
	ParticleManager:SetParticleControl(explode_particle, 61, Vector(1,0,0))
	ParticleManager:ReleaseParticleIndex(explode_particle)


	self.target:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_stunned", {duration = (1 - self.target:GetStatusResistance())*self:GetAbility().legendary_stun})
else 
...
if params.unit:GetUnitName() == "npc_psi_blades_crystal_mini" then 
	...
	damage = (params.original_damage) *(1 + self:GetAbility().crystal_damage)
else 
...
for _, enemy in pairs(enemies) do
    if enemy ~= params.unit and enemy:IsAlive() and enemy:GetUnitName() ~= "npc_psi_blades_crystal_mini" then
		...
		if params.unit:GetUnitName() == "npc_psi_blades_crystal_mini" and not enemy:IsMagicImmune() then 
		...
		if params.unit:GetUnitName() == "npc_psi_blades_crystal" then 
			enemy:AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_templar_assassin_psi_blades_custom_psi", {duration =  self:GetAbility().legendary_slow_duration*(1 - params.unit:GetStatusResistance())})
		end
...
if self:GetParent():HasModifier("modifier_templar_assassin_psiblades_4") then 
	...
	if random then 
		...
		local crystal = CreateUnitByName("npc_psi_blades_crystal_mini", pos, true, nil, nil, params.unit:GetTeamNumber())
		...
		crystal:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_kill", {duration = self:GetAbility().crystal_duration})
...
return
self:GetAbility().root_slow
end



modifier_templar_assassin_psi_blades_custom_psi = class({})
function modifier_templar_assassin_psi_blades_custom_psi:IsHidden() return false end
function modifier_templar_assassin_psi_blades_custom_psi:IsPurgable() return true end
function modifier_templar_assassin_psi_blades_custom_psi:GetTexture() return "buffs/psiblades_slow" end
function modifier_templar_assassin_psi_blades_custom_psi:GetEffectName() return "particles/void_astral_slow.vpcf" end

function modifier_templar_assassin_psi_blades_custom_psi:DeclareFunctions()
return
{
    MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE,
}
...
function modifier_templar_assassin_psi_blades_custom_psi:GetModifierMoveSpeedBonus_Percentage()
return
self:GetAbility().legendary_slow
end
...
function modifier_templar_assassin_psi_blades_custom_crystal:OnAttackLanded( param )
if not IsServer() then return end
