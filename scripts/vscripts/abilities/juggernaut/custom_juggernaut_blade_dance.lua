...
custom_juggernaut_blade_dance.mortal_damage = {60, 100}
custom_juggernaut_blade_dance.mortal_duration = 6
custom_juggernaut_blade_dance.mortal_health = 30

custom_juggernaut_blade_dance.speed_init = 5
custom_juggernaut_blade_dance.speed_inc = 5
custom_juggernaut_blade_dance.speed_duration = 3
custom_juggernaut_blade_dance.speed_max = 3

custom_juggernaut_blade_dance.heal = {0.2, 0.3, 0.4}

custom_juggernaut_blade_dance.parry_duration = 1
custom_juggernaut_blade_dance.parry_reduction = 0.4

custom_juggernaut_blade_dance.move_speed = 40
custom_juggernaut_blade_dance.move_duration = 2


custom_juggernaut_blade_dance.legendary_cd = 16
custom_juggernaut_blade_dance.legendary_range = 400
custom_juggernaut_blade_dance.legendary_slow_duration = 2
custom_juggernaut_blade_dance.legendary_slow = -80
custom_juggernaut_blade_dance.legendary_duration = 4
custom_juggernaut_blade_dance.legendary_resist = 50
custom_juggernaut_blade_dance.legendary_stun = 1
...
if not IsServer() then return end
  ...
  for _,enemy in pairs(enemies) do
    ...
    if angle_diff <= angle and not enemy:IsMagicImmune() then
      ...
      enemy:AddNewModifier(self:GetCaster(), self, "modifier_bashed", {duration = self.legendary_stun*(1 - enemy:GetStatusResistance())})
...
if self.record then 
    ...
    local heal = params.damage*(self:GetAbility().heal[self:GetParent():GetUpgradeStack("modifier_juggernaut_bladedance_lowhp")])
...
if not self.record or self.record ~= params.record then return end
  ...
  if self:GetParent():HasModifier("modifier_juggernaut_bladedance_double") then 
    ...
    self:GetParent():AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_custom_juggernaut_blade_dance_move", {duration = self:GetAbility().move_duration})
    params.target:AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_custom_juggernaut_blade_dance_move", {duration = self:GetAbility().move_duration})
...
if self:GetParent() == self:GetCaster() then 
  return self:GetAbility().move_speed
else 
  return -self:GetAbility().move_speed
end
...
if not IsServer() then return end
  ...
  self:SetStackCount(1)
  self.damage = self:GetAbility().mortal_damage[self:GetCaster():GetUpgradeStack("modifier_juggernaut_bladedance_stack")]
...
return (self:GetAbility().mortal_damage[self:GetCaster():GetUpgradeStack("modifier_juggernaut_bladedance_stack")])*self:GetStackCount() 
