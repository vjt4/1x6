...
function modifier_axe_battle_hunger_custom_debuff:OnCreated( kv )
	...
	self:SetStackCount( self:GetAbility():GetSpecialValueFor("armor_multiplier") )
...
if not IsServer() then return end
	...
	if self:GetCaster() == self:GetParent() then 
		...
		local heal = (self.damage + self:GetCaster():GetPhysicalArmorValue(false)*self:GetStackCount())*self:GetAbility().heal_pct
		...
		local damage = self.damage + self:GetCaster():GetPhysicalArmorValue(false)*self:GetStackCount()
