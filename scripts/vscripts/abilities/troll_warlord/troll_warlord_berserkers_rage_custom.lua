...
troll_warlord_berserkers_rage_custom.legendary_range = 0--100
...
function modifier_troll_warlord_berserkers_rage_custom:OnAttackLanded( params )
	...
	if RollPseudoRandomPercentage(chance, self:GetParent():entindex(), self:GetParent()) then
		...
		if self:GetParent():GetQuest() == "Troll.Quest_5" and params.target:IsRealHero() and not self:GetParent():QuestCompleted() then 
