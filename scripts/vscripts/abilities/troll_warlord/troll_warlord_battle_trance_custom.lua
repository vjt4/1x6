...
function modifier_troll_warlord_battle_trance_custom:OnIntervalThink()
	...
	if self.caster:GetAttackTarget() and 
		(self.target and not self.target:IsNull() and self.target:IsAlive() and not self.target:IsAttackImmune() and self.target:GetUnitName() ~= "npc_teleport" and not self.target:HasModifier("modifier_monkey_king_wukongs_command_custom_soldier"))
		 and  self.caster:GetAttackTarget():IsAlive() and not self.caster:GetAttackTarget():IsInvulnerable() and self.caster:CanEntityBeSeenByMyTeam(self.caster:GetAttackTarget()) then
	...
	if #hero_enemies > 0 then
		for enemy = 1, #hero_enemies do
			if hero_enemies[enemy] and self.caster:CanEntityBeSeenByMyTeam(hero_enemies[enemy]) and hero_enemies[enemy]:GetUnitName() ~= "npc_teleport"
			and not hero_enemies[enemy]:HasModifier("modifier_monkey_king_wukongs_command_custom_soldier") then
	...
	if #non_hero_enemies > 0 then
		for enemy = 1, #non_hero_enemies do
			if non_hero_enemies[enemy] and self.caster:CanEntityBeSeenByMyTeam(non_hero_enemies[enemy]) and non_hero_enemies[enemy]:GetUnitName() ~= "npc_teleport"
			 and not non_hero_enemies[enemy]:HasModifier("modifier_monkey_king_wukongs_command_custom_soldier") then
