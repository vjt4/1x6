...
monkey_king_jingu_mastery_custom.agility_bonus = {0.06, 0.09, 0.12}

monkey_king_jingu_mastery_custom.move_speed = {8, 12, 16}
monkey_king_jingu_mastery_custom.move_resist = {8, 12, 16}
...
monkey_king_jingu_mastery_custom.agility_stack = {3,4}
monkey_king_jingu_mastery_custom.agility_duration = 12
monkey_king_jingu_mastery_custom.agility_duration_creeps = 12
...
function modifier_monkey_king_jingu_mastery_custom_buff:IsPurgable() return true end
...
if ability and ability:GetLevel() > 0 and self:GetCaster():HasModifier("modifier_monkey_king_command_4") 
  ...
  ability:SpawnMonkeyKingPointScepter(point, ability.spells_duration, true)
end
...
if self:GetCaster():HasModifier("modifier_monkey_king_mastery_2") then 
  self.agi_percentage = self:GetAbility().agility_bonus[self:GetCaster():GetUpgradeStack("modifier_monkey_king_mastery_2")]
...
local hit_effect = ParticleManager:CreateParticle("particles/units/heroes/hero_monkey_king/monkey_king_quad_tap_hit.vpcf", PATTACH_ABSORIGIN_FOLLOW, params.unit)
ParticleManager:SetParticleControl(hit_effect, 1, params.unit:GetAbsOrigin())
ParticleManager:ReleaseParticleIndex(hit_effect)

if self:GetCaster():HasModifier("modifier_monkey_king_mastery_4") then 
  local duration = self:GetAbility().agility_duration
  if params.unit:IsCreep() then 
    duration = self:GetAbility().agility_duration_creeps
  end
...
return self:GetAbility().agility_stack[parent:GetUpgradeStack("modifier_monkey_king_mastery_4")]*self:GetStackCount()
end
