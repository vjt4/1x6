...
monkey_king_wukongs_command_custom.slow_max = 8
...
monkey_king_wukongs_command_custom.spells_duration = 4
...
self:GetCaster():EmitSound("Hero_MonkeyKing.FurArmy.Channel")
	...
	self.cast_particle = ParticleManager:CreateParticle("particles/units/heroes/hero_monkey_king/monkey_king_fur_army_cast.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetCaster())
	return true
end
...
function modifier_monkey_king_wukongs_command_custom_soldier_active:OnCreated(params)
	...
	if self:GetCaster():HasModifier("modifier_monkey_king_command_1") then 
	    self.attack_speed = self.attack_speed + self:GetAbility().attack_interval[self:GetCaster():GetUpgradeStack("modifier_monkey_king_command_1")]
	end
