...
for _,mod in pairs(self:GetParent():FindAllModifiers()) do
	...
	if mod:GetName() == "modifier_monkey_king_primal_spring_custom_legendary" then 
		local ability = self:GetParent():FindAbilityByName("monkey_king_primal_spring_custom")

		if ability then 
			mod:SetDuration(ability.legendary_duration_buff, true)
		end
	end
