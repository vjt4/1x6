...
if false and #my_game.ravager_table >= my_game.ravager_max then
...
if no_friends == true then 
	...
	if players[killer:GetTeamNumber()] and GameRules:GetDOTATime(false, false) >= patrol_second_tier and drop_orb == true then 

		self:GetParent().drop_orb = true

		local hero = players[killer:GetTeamNumber()]
