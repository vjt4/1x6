...
if hero:GetQuest() ~= nil then return end

local quest_table = {}
quest_table.name = kv.name


local quests_data = CustomNetTables:GetTableValue("hero_quests", tostring(kv.PlayerID));
...
for hero_name,quest in pairs(quests_data) do 
	...
	if hero_name == hero:GetUnitName() then 
		...
		for _,hero_quest in pairs(quest) do 
			if hero_quest.name == kv.name then 
