...
if thisEntity.ice:IsFullyCastable() then
    ...
    if (thisEntity:GetAttackTarget() ~= nil) and thisEntity:GetHealthPercent() <= thisEntity.ice:GetSpecialValueFor("health") 
...
if IsValidEntity(enemy_for_attack[1]) 
    and not enemy_for_attack[1]:IsCourier() 
    and enemy_for_attack[1]:GetUnitName() ~= "npc_teleport" 
    and not enemy_for_attack[1]:HasModifier("modifier_monkey_king_wukongs_command_custom_soldier") 
    and not enemy_for_attack[1]:HasModifier("modifier_monkey_king_mischief_custom")
...
for _, target in pairs(enemy_for_attack) do
    ...
    and not target:HasModifier("modifier_monkey_king_wukongs_command_custom_soldier")
    and not target:HasModifier("modifier_monkey_king_mischief_custom")
